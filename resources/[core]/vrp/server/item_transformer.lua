...
local function tr_add_player(tr, player, recipe) -- add player to transforming
	...
	vRPclient.setProgressBar(
		player,
		{ "vRP:tr:" .. tr.name, "botright", recipe .. "...", tr.itemtr.r, tr.itemtr.g, tr.itemtr.b, 0 }
	)
...
function vRP.setItemTransformer(name, itemtr)
	...
	-- build menu
	tr.menu = { name = itemtr.name, css = {
		top = "75px",
		header_color = "rgba(" .. itemtr.r .. "," .. itemtr.g .. "," .. itemtr.b .. ",0.75)",
	} }
...
local function transformers_tick()
	...
	SetTimeout(4250, transformers_tick)
end
transformers_tick()
...
-- build informer menu
local informer_menu =
	{ name = lang.itemtr.informer.title(), css = { top = "75px", header_color = "rgba(0,255,125,0.75)" } }
