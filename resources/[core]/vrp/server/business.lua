...
local function open_business_directory(player, page) -- open business directory with pagination system
	...
	local menu = {
		...
		css = { top = "75px", header_color = "rgba(24, 24, 24, 0.75)" },
	}
			...
			for k, v in pairs(rows) do
				...
				if row.user_id ~= nil then
					...
					vRP.getUserIdentity(row.user_id, function(identity)
						...
						count = count - 1
						if count == 0 then
							menu[lang.business.directory.dnext()] = {
								function()
									open_business_directory(player, page + 1)
								end,
							}
							menu[lang.business.directory.dprev()] = {
								function()
									open_business_directory(player, page - 1)
								end,
							}
							...
							if amount > 0 then
								if vRP.tryPayment(user_id, amount) then
									MySQl.Async.execute(
										...
										{ user_id = user_id, capital = amount }
									)
								...
								if amount > 0 and amount <= launder_left then
									if vRP.tryGetInventoryItem(user_id, "dirty_money", amount, false) then
										...
										MySQL.Async.execute(
											...
											{ user_id = user_id, laundered = amount }
										)
										...
										if capital >= cfg.minimum_capital then
											if vRP.tryPayment(user_id, capital) then
												MySQL.Async.execute(
													...
													{ user_id = user_id, name = name, capital = capital, time = os.time() }
												)
												vRPclient.notify(player, { lang.business.open.created() })

												local dmessage = "```ID: "
													.. user_id
													.. " har oprettet en virksomhed ("
													.. name
													.. ") med et start kapital på "
													.. capital
													.. " DKK```"
												PerformHttpRequest(
