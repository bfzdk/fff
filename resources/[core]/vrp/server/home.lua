...
--MySQL.Async.execute("vRP/home_tables", [[
...
function vRP.getUserAddress(user_id, cbr)
	...
	MySQL.Async.fetchAll(
		"SELECT home, number FROM vrp_user_homes WHERE user_id = @user_id",
		{ user_id = user_id },
...
function vRP.setUserAddress(user_id, home, number)
	MySQL.Async.execute(
		...
		{ user_id = user_id, home = home, number = number }
	)
end
...
function vRP.getUserByAddress(home, number, cbr)
	...
	MySQL.Async.fetchAll(
		"SELECT user_id FROM vrp_user_homes WHERE home = @home AND number = @number",
