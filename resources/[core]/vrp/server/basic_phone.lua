		...
		for k, v in pairs(players) do
			...
			TriggerClientEvent(
				...
				v,
				{
					...
					sounds = { sources = { "panikknap.ogg" }, volume = 0.1, conditions = { "docVisible" } },
				}
			)
...
-- build announce menu
local announce_menu =
	{ name = lang.phone.announce.title(), css = { top = "75px", header_color = "rgba(133,189,191,0.75)" } }
				...
				if string.len(msg) > 10 and string.len(msg) < 1000 then
					if announce.price <= 0 or vRP.tryFullPayment(user_id, announce.price) then -- try to pay the announce
						TriggerClientEvent(
							...
							player,
							{
								text = { lang.money.paid({ announce.price }) },
								type = "success",
								queue = "global",
								timeout = 4000,
								layout = "bottomCenter",
