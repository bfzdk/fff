...
local cfg = vRP.module("cfg/aptitudes")
local lang = vRP.lang
	...
	if user_id ~= nil then
		if player_apts[player] then -- hide
			...
			for k, v in pairs(uaptitudes) do
				-- display group
				content = '<div class="apptitudesheader">'
					.. content
					.. lang.aptitude.display.group({ vRP.getAptitudeGroupTitle(k) })
					.. "</div>"
				for l, w in pairs(v) do
					...
					if def then
						...
						content = content
							...
							.. (percent / 100)
							.. '" data-color="rgba(0,125,255,0.7)" data-bgcolor="rgba(0,125,255,0.3)">'
							.. lang.aptitude.display.aptitude({ def[1], exp, lvl, percent })
							.. "</div>"
					end
				end
			end
