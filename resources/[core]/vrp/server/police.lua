...
function vRP.setLicense(user_id, dmvtest)
	MySQL.Async.execute(
		"UPDATE vrp_users SET DmvTest = @dmvtest WHERE id = @user_id",
		{ user_id = user_id, dmvtest = dmvtest }
	)
end
					...
					if identity then
						...
						vRP.getUserBusiness(user_id, function(business)
							...
							vRP.getUserAddress(user_id, function(address)
								...
								local content =
									lang.police.identity.info({ name, firstname, age, registration, phone, bname, bcapital, home, number })
								vRPclient.setDiv(
									player,
									{
										"police_pc",
										".div_police_pc{ background-color: rgba(0,0,0,0.75); color: white; font-family: Roboto; width: 500px; padding: 10px; margin: auto; margin-top: 150px; }",
										content,
									}
								)
							end)
...
menu_pc[lang.police.pc.records.delete.title()] =
	{ ch_delete_police_records, lang.police.pc.records.delete.description() }
...
local choice_dragplayer = {
	...
	lang.police.menu.dragplayer.description(),
}

local choice_dragplayer_ems = {
	function(player, choice)
		vRPclient.getNearestPlayer(player, { 3 }, function(nplayer)
			local nuser_id = vRP.getUserId(nplayer)
			if nuser_id ~= nil then
								...
								vRP.getUserBusiness(nuser_id, function(business)
									...
									vRP.getUserAddress(nuser_id, function(address)
										...
										vRPclient.setDiv(
											...
											{
												"police_identity",
												".div_police_identity{ background-color: rgba(0,0,0,0.75); color: white; font-weight: bold; width: 500px; padding: 10px; margin: auto; margin-top: 150px; }",
												content,
											}
										)
										-- request to hide div
									...
									if handcuffed then
										table.insert(
											...
											"**"
												...
												.. " ("
												.. user_id
												.. ")** beslaglagde følgende våben fra **"
												.. civ_name
												.. " "
												.. civ_lname
												.. " ("
										...
										vRPclient.replaceWeapons(nplayer, {}, function(weapons)
											...
											PerformHttpRequest(
												...
												"POST",
												json.encode({
													username = "DevoNetwork [POLITI]" .. GetConvar("servernumber", "#1"),
													content = table.concat(seized_guns),
												}),
									...
									if handcuffed then
										table.insert(
											...
											"**"
												...
												.. " ("
												.. user_id
												.. ")** beslaglagde følgende genstande fra **"
												.. civ_name
												.. " "
												.. civ_lname
												.. " ("
										...
										PerformHttpRequest(
											...
											json.encode({
												username = "DevoNetwork [POLITI]" .. GetConvar("servernumber", "#1"),
												content = table.concat(seized_items),
											}),
											{ ["Content-Type"] = "application/json" }
										)
...
local ch_confiscade_vehicle = {
	function(player, choice)
		TriggerClientEvent("Kian_impound:impoundcar", player)
	end,
	"Beslaglæg en spillers køretøj",
}

-- Frigiv et køretøj
local ch_free_vehicle = {
	function(player, choice)
				...
				vRP.prompt(player, "Hvad er modellen på køretøjet du vil frigive?", "", function(player, vehicle)
					if vehicle ~= "" then
						MySQL.Async.fetchAll(
							"SELECT vehicle FROM vrp_user_vehicles WHERE vehicle = @vehicle AND vehicle_plate = @plate",
...
local choice_license = {
	...
	lang.police.menu.license.description(),
}

-- search identity by numbmer plate on foot
local choice_carsearch = {
	function(player, choice)
							...
							vRP.getUserBusiness(user_id, function(business)
								...
								vRP.getUserAddress(user_id, function(address)
									...
									vRPclient.setDiv(
										...
										{
											"police_identity",
											".div_police_identity{ background-color: rgba(0,0,0,0.75); color: white; font-family: Roboto; width: 500px; padding: 10px; margin: auto; margin-top: 150px; }",
											content,
										}
									)
									vRP.request(player, lang.police.menu.check.request_hide(), 1000, function(player, ok)
	...
	lang.police.menu.searchcar.description(),
}

-- show police records by cpr on foot
local choice_cprsearch = {
	function(player, choice)
		vRP.prompt(player, lang.police.pc.searchreg.prompt(), "", function(player, reg)
			vRP.getUserByRegistration(reg, function(user_id)
				if user_id ~= nil then
	...
	lang.police.menu.cprsearch.description(),
}
				...
				vRP.getUserIdentity(user_id, function(identity)
					...
					if identity then
						...
						PerformHttpRequest(
							...
							json.encode({
								username = "DevoNetwork [POLITI]" .. GetConvar("servernumber", "#1"),
								content = table.concat(stored_items),
							}),
							{ ["Content-Type"] = "application/json" }
						)
...
local choice_heal = {
	...
	lang.emergency.menu.heal.description(),
}
				...
				for l, w in pairs(listeners) do -- each listening player
					...
					if lplayer ~= nil then
						vRPclient.setNamedBlip(
							lplayer,
							{ "vRP:wanted:" .. k, x, y, z, cfg.wanted.blipid, cfg.wanted.blipcolor, lang.police.wanted({ v }) }
						)
					end
				end
			end)
	...
	if vRP.hasPermission(user_id, "police.pc") then
		...
		PerformHttpRequest(
			...
			json.encode({
				...
				content = "ID: " .. user_id .. ' Forsøgte at trigger "handcuff:cuffHim"',
			}),
			{ ["Content-Type"] = "application/json" }
		)
	end
