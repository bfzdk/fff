	...
	while true do
		...
		if health <= cfg.coma_threshold and coma_left > 0 then
			if not in_coma then -- go to coma state
				...
				vRP.playScreenEffect(cfg.coma_effect, -1)
				vRP.ejectVehicle()
				vRP.setRagdoll(true)
			else -- in coma
				-- maintain life
			...
			drawTxt(
				...
				1.0,
				0.65,
				"Du kan genopstå om: ~r~" .. coma_left .. " ~w~sekunder.",
				255,
				255,
				255,
				255
		...
		DisablePlayerFiring(GetPlayerPed(-1), true) -- Disable weapon firing
			...
			if in_coma then -- get out of coma state
				...
				vRP.stopScreenEffect(cfg.coma_effect)
