...
function vRP.getUserIdByIdentifiers(ids, cbr)
	...
	if ids ~= nil and #ids then
		...
		local function search()
			...
			if i <= #ids then
				if
					(not config.ignore_ip_identifier or (string.find(ids[i], "ip:") == nil))
					and (not config.ignore_license_identifier or (string.find(ids[i], "license:") == nil))
					and (not config.ignore_xbox_identifier or (string.find(ids[i], "xbl:") == nil))
					and (not config.ignore_discord_identifier or (string.find(ids[i], "discord:") == nil))
					and (not config.ignore_live_identifier or (string.find(ids[i], "live:") == nil))
				then
					validids = validids + 1
						...
						function(rows, affected)
							if next_id then
								...
								for l, w in pairs(ids) do
									if
										(not config.ignore_ip_identifier or (string.find(w, "ip:") == nil))
										and (not config.ignore_license_identifier or (string.find(w, "license:") == nil))
										and (not config.ignore_xbox_identifier or (string.find(w, "xbl:") == nil))
										and (not config.ignore_discord_identifier or (string.find(w, "discord:") == nil))
										and (not config.ignore_live_identifier or (string.find(w, "live:") == nil))
...
function vRP.isWhitelisted(user_id, cbr)
	...
	MySQL.Async.fetchAll(
		"SELECT whitelisted FROM vrp_users WHERE id = @user_id",
		{ user_id = user_id },
		function(rows, affected)
			if #rows > 0 then
...
function vRP.getLastLogin(user_id, cbr)
	local task = Task(cbr, { "" })
	MySQL.Async.fetchAll(
		"SELECT last_login FROM vrp_users WHERE id = @user_id",
		{ user_id = user_id },
		function(rows, affected)
			if #rows > 0 then
...
AddEventHandler("playerConnecting", function(name, setMessage, deferrals)
	...
	if isStopped == false then
		...
		print(
			"(" .. vRP.getPlayerEndpoint(source) .. ") blev kicket for at joine imens serveren er igang med at " .. isStopped
		)
