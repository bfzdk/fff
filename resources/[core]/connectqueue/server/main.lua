...
local function playerConnect(name, setKickReason, deferrals)
	...
	update(
		string_format(
			Config.Language.pos
				.. ((Queue:TempSize() and Config.ShowTemp) and " (" .. Queue:TempSize() .. " temp)" or "00:00:00"),
			pos,
			Queue:GetSize(),
			""
		)
	...
	while true do
		...
		local msg = string_format(
			Config.Language.pos .. ((Queue:TempSize() and Config.ShowTemp) and " (" .. Queue:TempSize() .. " temp)" or ""),
			pos,
			Queue:GetSize(),
			qTime
		)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		while i <= Queue:ConnectingSize() do
			...
			if
				((data.timeout >= 300 and not endPoint) or data.timeout >= Config.ConnectTimeOut)
				and data.source ~= "debug"
				and os_time() - data.firstconnect > 5
			then
				remove(data)
				Queue:DebugPrint(
