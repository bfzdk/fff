	...
	for i = 1, paramsNum do
		...
		if not value and (not param.optional or param.optional and arg) then
			return Citizen.Trace(
				("^1command '%s' received an invalid %s for argument %s (%s), received '%s'^0\n"):format(
					string.strsplit(" ", raw) or raw,
					param.type,
					i,
					param.name,
					arg
...
function lib.addCommand(commandName, properties, cb, ...)
	...
	if properties then
		if ... or table.type(properties) ~= "hash" then
			...
			warn(
				("command '%s' is using deprecated syntax for lib.addCommand\nupdate the command or use lib.__addCommand to ignore this warning\n> source ^0(^5%s^0:%d)"):format(
					_commandName,
					info.short_src,
					info.currentline
				)
