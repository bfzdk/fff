...
local function getTriangles(polygon)
	...
	while polygonN - #triangles > 2 do
		...
		if
			polygon:containsSegment(
				vec3(glm.segment2d.getPoint(a2d, c2d, 0.01), zValue),
				vec3(glm.segment2d.getPoint(a2d, c2d, 0.99), zValue)
			)
		then
...
CreateThread(function()
	...
	while true do
		...
		local zones = lib.grid.getNearbyEntries(coords, function(entry)
			return entry.remove == removeZone
		end) --[[@as Array<CZone>]]
...
local function setDebug(self, bool, colour)
	...
	if bool and self.debug and self.debug ~= true then
		return
	end

	self.triangles = self.__type == "poly" and getTriangles(self.polygon)
		or self.__type == "box" and {
...
function lib.zones.getAllZones()
	return Zones
end

function lib.zones.getCurrentZones()
	return insideZones
end

function lib.zones.getNearbyZones()
	return nearbyZones
end
