...
function math.lerp(start, finish, duration)
	...
	if
		typeStart ~= "number"
		and typeStart ~= "vector2"
		and typeStart ~= "vector3"
		and typeStart ~= "vector4"
		and typeStart ~= "table"
	then
		error(
			("expected argument 1 to have type '%s' (received %s)"):format(
				"number | table | vector2 | vector3 | vector4",
				typeStart
			)
		)
	end
