...
function lib.getRelativeCoords(coords, rotation, offset)
	if type(rotation) == "vector3" and offset then
		...
		return vec3(coords.x + rotatedX, coords.y + rotatedY, coords.z + rotatedZ)
	end
	...
	return coords.w and vec4(x + relativeX, y + relativeY, z + offset.z, w)
		or vec3(x + relativeX, y + relativeY, z + offset.z)
end
