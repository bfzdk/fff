...
function OxTask:getNextTime()
	...
	if weekday and weekday ~= currentDate.wday then
		return
	end
	...
	if self.lastRun and nextTime - self.lastRun < 60 then
		if self.debug then
			lib.print.debug(
				("Preventing duplicate execution of task %s - Last run: %s, Next scheduled: %s"):format(
					self.id,
...
function OxTask:scheduleTask()
	...
	if sleep < 0 then
		...
		if self.debug then
			lib.print.debug(
				("Task %s is %s seconds overdue, executing now due to maxDelay=%s"):format(self.id, -sleep, self.maxDelay)
			)
		end
