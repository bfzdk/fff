...
function GetVehicleInDirectionSphere(entFrom, coordFrom, coordTo)
	local rayHandle =
		StartShapeTestCapsule(coordFrom.x, coordFrom.y, coordFrom.z, coordTo.x, coordTo.y, coordTo.z, 2.0, 10, entFrom, 7)
		...
		if GetVehicleClass(vehicle) == 18 then
			...
			if radarEnabled then
				TriggerEvent(
					...
					{
						text = "Rader <b style='color:#00ff00'>aktiveret",
						type = "info",
						timeout = 5000,
						layout = "centerRight",
						queue = "global",
				...
				TriggerEvent(
					"pNotify:SendNotification",
					{
						text = "Rader <b style='color:#ff0000'>deaktiveret",
						type = "info",
						timeout = 5000,
						layout = "centerRight",
						queue = "global",
			...
			TriggerEvent(
				"pNotify:SendNotification",
				{
					text = "Du skal sidde i en politibil ",
					type = "error",
					timeout = 5000,
					layout = "centerRight",
					queue = "global",
...
function ToggleSpeedType()
	if radarInfo.speedType == "mph" then
		...
		TriggerEvent(
			"pNotify:SendNotification",
			{
				text = "Fart sat til <b style='color:#ffff00'> KM/T âœ…",
				type = "info",
				timeout = 5000,
				layout = "centerRight",
				queue = "global",
		...
		TriggerEvent(
			"pNotify:SendNotification",
			{
				text = "Fart sat til <b style='color:#ffff00'> MPH âœ…",
				type = "info",
				timeout = 5000,
				layout = "centerRight",
				queue = "global",
	...
	if radarEnabled then
		...
		if IsPedSittingInAnyVehicle(ped) then
			...
			if GetPedInVehicleSeat(vehicle, -1) == ped and GetVehicleClass(vehicle) == 18 then
				...
				-- Front Antenna
				if radarInfo.fwdXmit then
					local forwardPosition = GetOffsetFromEntityInWorldCoords(
						vehicle,
				...
				-- Rear Antenna
				if radarInfo.bwdXmit then
					local backwardPosition = GetOffsetFromEntityInWorldCoords(
						vehicle,
...
Citizen.CreateThread(function()
	while true do
		...
		if
			((not inVeh or (inVeh and GetVehicleClass(veh) ~= 18)) and radarEnabled and not hidden)
			or IsPauseMenuActive() and radarEnabled
		then
			hidden = true
