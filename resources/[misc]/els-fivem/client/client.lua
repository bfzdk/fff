...
Citizen.CreateThread(function()
	...
	while true do
		if vehInTable(els_Vehicles, checkCarHash(GetVehiclePedIsUsing(GetPlayerPed(-1)))) then
			if
				(GetPedInVehicleSeat(GetVehiclePedIsUsing(GetPlayerPed(-1)), -1) == GetPlayerPed(-1))
				or (GetPedInVehicleSeat(GetVehiclePedIsUsing(GetPlayerPed(-1)), 0) == GetPlayerPed(-1))
			then
				...
				if GetLastInputMethod(0) then
					...
					if IsDisabledControlPressed(0, keyboard.modifyKey) then
						if
							IsDisabledControlPressed(0, keyboard.modifyKey) and IsDisabledControlJustReleased(0, keyboard.pattern.primary)
						then
						...
						if
							IsDisabledControlPressed(0, keyboard.modifyKey) and IsDisabledControlJustReleased(0, keyboard.pattern.secondary)
						then
							if playButtonPressSounds then
								PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
							end
					...
					if doesVehicleHaveTrafficAdvisor(GetVehiclePedIsUsing(GetPlayerPed(-1))) then
						if IsDisabledControlPressed(0, keyboard.modifyKey) then
							if
								IsDisabledControlPressed(0, keyboard.modifyKey) and IsDisabledControlJustReleased(0, keyboard.pattern.advisor)
							then
								if playButtonPressSounds then
									PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
								end
						...
						if elsVehs[GetVehiclePedIsUsing(GetPlayerPed(-1))] ~= nil then
							if elsVehs[GetVehiclePedIsUsing(GetPlayerPed(-1))].stage == 3 then
								if IsDisabledControlJustReleased(0, keyboard.siren.tone_one) then
									...
									if
										m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 2
										or m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 3
										or m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 0
									then
								...
								if IsDisabledControlJustReleased(0, keyboard.siren.tone_two) then
									...
									if
										m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 1
										or m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 3
										or m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 0
									then
								...
								if IsDisabledControlJustReleased(0, keyboard.siren.tone_three) then
									...
									if
										m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 1
										or m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 2
										or m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 0
									then
					...
					if els_Vehicles[checkCarHash(GetVehiclePedIsUsing(GetPlayerPed(-1)))].activateUp then
						...
						elseif
							IsDisabledControlPressed(0, keyboard.modifyKey) and IsDisabledControlJustReleased(0, keyboard.stageChange)
						then
							if playButtonPressSounds then
								PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
							end
					...
					if els_Vehicles[checkCarHash(GetVehiclePedIsUsing(GetPlayerPed(-1)))].activateUp then
						if
							IsDisabledControlPressed(0, controller.modifyKey) and IsDisabledControlJustReleased(0, controller.stageChange)
						then
							if playButtonPressSounds then
								PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
							end
					...
					if IsDisabledControlPressed(0, controller.modifyKey) then
						...
						if
							IsDisabledControlPressed(0, controller.modifyKey) and IsDisabledControlJustReleased(0, controller.takedown)
						then
							if playButtonPressSounds then
								PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
							end
						...
						if elsVehs[GetVehiclePedIsUsing(GetPlayerPed(-1))] ~= nil then
							if elsVehs[GetVehiclePedIsUsing(GetPlayerPed(-1))].stage == 3 then
								...
								if IsDisabledControlPressed(0, controller.modifyKey) then
									...
									if dualEnable[GetVehiclePedIsUsing(GetPlayerPed(-1))] then
										if
											IsDisabledControlPressed(0, controller.modifyKey)
											and IsDisabledControlJustReleased(0, controller.siren.tone_one)
										then
											if playButtonPressSounds then
												PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
											end
										...
										if
											IsDisabledControlPressed(0, controller.modifyKey)
											and IsDisabledControlJustReleased(0, controller.siren.tone_two)
										then
											if playButtonPressSounds then
												PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
											end
										...
										if
											IsDisabledControlPressed(0, controller.modifyKey)
											and IsDisabledControlJustReleased(0, controller.siren.tone_three)
										then
											if playButtonPressSounds then
												PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
											end
	...
	while true do
		if guiEnabled and vehInTable(els_Vehicles, checkCarHash(GetVehiclePedIsUsing(GetPlayerPed(-1)))) then
			...
			then
				...
				_DrawRect(0.920, 0.947, 0.15, 0.09, 0, 0, 0, 200, 0)

				if getVehicleLightStage(GetVehiclePedIsUsing(GetPlayerPed(-1))) == 1 then
					_DrawRect(0.865, 0.920, 0.03, 0.02, 173, 0, 0, 225, 0)
					Draw("1", 0, 0, 0, 255, 0.865, 0.910, 0.25, 0.25, 1, true, 0)
				else
					_DrawRect(0.865, 0.920, 0.03, 0.02, 186, 186, 186, 225, 0)
					Draw("1", 0, 0, 0, 255, 0.865, 0.910, 0.25, 0.25, 1, true, 0)
				end

				if getVehicleLightStage(GetVehiclePedIsUsing(GetPlayerPed(-1))) == 2 then
					_DrawRect(0.900, 0.920, 0.03, 0.02, 173, 0, 0, 225, 0)
					Draw("2", 0, 0, 0, 255, 0.900, 0.910, 0.25, 0.25, 1, true, 0)
				else
					_DrawRect(0.900, 0.920, 0.03, 0.02, 186, 186, 186, 225, 0)
					Draw("2", 0, 0, 0, 255, 0.900, 0.910, 0.25, 0.25, 1, true, 0)
				end

				if getVehicleLightStage(GetVehiclePedIsUsing(GetPlayerPed(-1))) == 3 then
					_DrawRect(0.935, 0.920, 0.03, 0.02, 173, 0, 0, 225, 0)
					Draw("3", 0, 0, 0, 255, 0.935, 0.910, 0.25, 0.25, 1, true, 0)
				else
					_DrawRect(0.935, 0.920, 0.03, 0.02, 186, 186, 186, 225, 0)
					Draw("3", 0, 0, 0, 255, 0.935, 0.910, 0.25, 0.25, 1, true, 0)
				end

				_DrawRect(0.973, 0.920, 0.035, 0.02, 186, 186, 186, 225, 0)
				Draw("PRIM " .. tostring(lightPatternPrim), 0, 0, 0, 255, 0.9735, 0.910, 0.25, 0.25, 1, true, 0)

				_DrawRect(0.973, 0.947, 0.035, 0.02, 186, 186, 186, 225, 0)
				Draw("SEC " .. tostring(lightPatternSec), 0, 0, 0, 255, 0.9735, 0.936, 0.25, 0.25, 1, true, 0)

				if doesVehicleHaveTrafficAdvisor(GetVehiclePedIsUsing(GetPlayerPed(-1))) then -- Change
					_DrawRect(0.973, 0.975, 0.035, 0.02, 186, 186, 186, 225, 0)
					Draw("ADV " .. tostring(advisorPatternSelectedIndex), 0, 0, 0, 255, 0.973, 0.964, 0.25, 0.25, 1, true, 0)
				end

				if h_horn_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 1 then
					_DrawRect(0.865, 0.947, 0.03, 0.02, 0, 173, 0, 225, 0)
				else
					_DrawRect(0.865, 0.947, 0.03, 0.02, 186, 186, 186, 225, 0)
				end
				Draw("HORN", 0, 0, 0, 255, 0.865, 0.937, 0.25, 0.25, 1, true, 0)

				if dualEnable[GetVehiclePedIsUsing(GetPlayerPed(-1))] then
					_DrawRect(0.900, 0.947, 0.03, 0.02, 0, 213, 255, 225, 0)
				else
					_DrawRect(0.900, 0.947, 0.03, 0.02, 186, 186, 186, 225, 0)
				end
				Draw("DUAL", 0, 0, 0, 255, 0.900, 0.936, 0.25, 0.25, 1, true, 0)

				if IsVehicleExtraTurnedOn(GetVehiclePedIsUsing(GetPlayerPed(-1)), 11) then
					_DrawRect(0.935, 0.947, 0.03, 0.02, 255, 0, 0, 255, 0)
					Draw("TKD", 0, 0, 0, 255, 0.935, 0.947, 0.25, 0.25, 1, true, 0)
				else
					_DrawRect(0.935, 0.947, 0.03, 0.02, 186, 186, 186, 225, 0)
					Draw("TKD", 0, 0, 0, 255, 0.935, 0.936, 0.25, 0.25, 1, true, 0)
				end

				if m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 1 then
					if d_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 1 then
						_DrawRect(0.865, 0.975, 0.015, 0.02, 0, 173, 0, 225, 0)
					else
						_DrawRect(0.865, 0.975, 0.03, 0.02, 0, 173, 0, 225, 0)
					end
				else
					_DrawRect(0.865, 0.975, 0.03, 0.02, 186, 186, 186, 225, 0)
				end

				if d_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 1 then
					if m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 1 then
						_DrawRect(0.865, 0.975, 0.015, 0.02, 0, 213, 255, 225, 2)
					else
						_DrawRect(0.865, 0.975, 0.03, 0.02, 0, 213, 255, 225, 0)
					end
				end

				Draw("MAIN", 0, 0, 0, 255, 0.865, 0.964, 0.25, 0.25, 3, true, 0)

				if m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 2 then
					if d_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 2 then
						_DrawRect(0.900, 0.975, 0.015, 0.02, 0, 173, 0, 225, 0)
					else
						_DrawRect(0.900, 0.975, 0.03, 0.02, 0, 173, 0, 225, 0)
					end
				else
					_DrawRect(0.900, 0.975, 0.03, 0.02, 186, 186, 186, 225, 0)
				end

				if d_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 2 then
					if m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 2 then
						_DrawRect(0.900, 0.975, 0.015, 0.02, 0, 213, 255, 225, 2)
					else
						_DrawRect(0.900, 0.975, 0.03, 0.02, 0, 213, 255, 255, 0)
					end
				end

				Draw("SEC", 0, 0, 0, 255, 0.900, 0.964, 0.25, 0.25, 3, true, 0)

				if m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 3 then
					if d_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 3 then
						_DrawRect(0.935, 0.975, 0.03, 0.02, 0, 173, 0, 225, 0)
					else
						_DrawRect(0.935, 0.975, 0.03, 0.02, 0, 173, 0, 225, 0)
					end
				else -- 0.935, 0.975, 0.03, 0.02
					_DrawRect(0.935, 0.975, 0.03, 0.02, 186, 186, 186, 225, 0)
				end

				if d_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 3 then
					if m_siren_state[GetVehiclePedIsUsing(GetPlayerPed(-1))] == 3 then
						_DrawRect(0.935, 0.975, 0.03, 0.02, 0, 213, 255, 225, 2)
					else
						_DrawRect(0.935, 0.975, 0.03, 0.02, 0, 213, 255, 225, 0)
					end
				end

				Draw("AUX", 0, 0, 0, 255, 0.935, 0.964, 0.25, 0.25, 3, true, 0)
			end -- _DrawRect(0.97, 0.925, 0.035, 0.02, 186, 186, 186, 225, 0)
		end
		Wait(1)
	end
end)
	...
	while true do
		for k, v in pairs(elsVehs) do
			if v ~= nil or DoesEntityExist(k) then
				if
					GetDistanceBetweenCoords(GetEntityCoords(k, true), GetEntityCoords(GetPlayerPed(-1), true), true)
					<= vehicleSyncDistance
				then
									...
									if els_Vehicles[vehN].extras[i].env_light then
										if i == 11 then
											DrawSpotLightWithShadow(
												coords.x + els_Vehicles[vehN].extras[11].env_pos.x,
												coords.y + els_Vehicles[vehN].extras[11].env_pos.y,
												coords.z + els_Vehicles[vehN].extras[11].env_pos.z,
												rotX,
												rotY,
												rotZ,
												255,
				...
				if doesVehicleHaveTrafficAdvisor(k) then
					...
					then
						...
						if vehIsReady[k] then
							runPatternAdvisor(k, v.stage, v.advisorPattern, function(cb)
								vehIsReady[k] = cb
							end)
						end
					end
			...
			if v ~= nil or DoesEntityExist(k) then
				...
				elseif v.stage == 2 then
					...
					then
						...
						if vehIsReady[k] then
							runPatternStageTwo(k, v.secPattern, function(cb)
								vehIsReady[k] = cb
							end)
						end
					end
			...
			if v ~= nil or DoesEntityExist(k) then
				if v.stage == 3 then
					...
					then
						...
						if vehIsReady[k] then
							runPatternStageThree(k, v.primPattern, function(cb)
								vehIsReady[k] = cb
							end)
						end
					end
