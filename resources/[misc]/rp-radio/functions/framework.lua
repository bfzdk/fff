	...
	if true then
		...
		if Scully.UseItem then
			AddEventHandler(
				...
				function(source, name, slotId, metadata) -- OX_Inventory -- Gjordt så det fungere med nyt inv (kian)
		...
		Scully.Functions = {
			GetPlayerName = function(source)
				...
				if xPlayer.character then
					return xPlayer.character.callsign .. " | ",
						string.sub(xPlayer.character.firstname, 1, 1) .. ". " .. xPlayer.character.lastname
				else
					return "", GetPlayerName(source)
				end
			end,
