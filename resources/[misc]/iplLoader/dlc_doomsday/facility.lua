...
DoomsdayFacility = {
	...
	Lounge = {
		...
		Clear = function(refresh)
			SetIplPropState(
				DoomsdayFacility.interiorId,
				{ DoomsdayFacility.Lounge.utility, DoomsdayFacility.Lounge.prestige, DoomsdayFacility.Lounge.premier },
				false,
				refresh
			)
		end,
	...
	Security = {
		...
		Clear = function(refresh)
			SetIplPropState(
				DoomsdayFacility.interiorId,
				{ DoomsdayFacility.Security.off, DoomsdayFacility.Security.on },
				false,
				refresh
			)
		end,
	...
	Cannon = {
		...
		Clear = function(refresh)
			SetIplPropState(
				DoomsdayFacility.interiorId,
				{ DoomsdayFacility.Cannon.off, DoomsdayFacility.Cannon.on },
				false,
				refresh
			)
		end,
	...
	PrivacyGlass = {
		...
		Bedroom = {
			Enable = function(state)
				local handle =
					GetClosestObjectOfType(367.99, 4827.745, -59.0, 1.0, GetHashKey("xm_prop_x17_l_glass_03"), false, false, false)
			...
			Control = {
				...
				Enable = function(state)
					local handle = GetClosestObjectOfType(
						DoomsdayFacility.PrivacyGlass.Bedroom.Control.position.x,
						DoomsdayFacility.PrivacyGlass.Bedroom.Control.position.y,
						DoomsdayFacility.PrivacyGlass.Bedroom.Control.position.z,
						1.0,
					...
					if state then
						if handle == 0 then
							...
							local privacyGlass = CreateObjectNoOffset(
								DoomsdayFacility.PrivacyGlass.controlModelHash,
								DoomsdayFacility.PrivacyGlass.Bedroom.Control.position.x,
								DoomsdayFacility.PrivacyGlass.Bedroom.Control.position.y,
								DoomsdayFacility.PrivacyGlass.Bedroom.Control.position.z,
							...
							SetEntityRotation(
								privacyGlass,
								DoomsdayFacility.PrivacyGlass.Bedroom.Control.rotation.x,
								DoomsdayFacility.PrivacyGlass.Bedroom.Control.rotation.y,
								DoomsdayFacility.PrivacyGlass.Bedroom.Control.rotation.z,
								2,
								true
							)
		...
		Lounge = {
			Glasses = {
				{
					modelHash = GetHashKey("xm_prop_x17_l_door_glass_01"),
					entityHash = GetHashKey("xm_prop_x17_l_door_frame_01"),
					entityPos = { x = 359.22, y = 4846.043, z = -58.85 },
				},
				{
					modelHash = GetHashKey("xm_prop_x17_l_door_glass_01"),
					entityHash = GetHashKey("xm_prop_x17_l_door_frame_01"),
					entityPos = { x = 369.066, y = 4846.273, z = -58.85 },
				},
				{
					modelHash = GetHashKey("xm_prop_x17_l_glass_01"),
					entityHash = GetHashKey("xm_prop_x17_l_frame_01"),
					entityPos = { x = 358.843, y = 4845.103, z = -60.0 },
				},
				{
					modelHash = GetHashKey("xm_prop_x17_l_glass_02"),
					entityHash = GetHashKey("xm_prop_x17_l_frame_02"),
					entityPos = { x = 366.309, y = 4847.281, z = -60.0 },
				},
				{
					modelHash = GetHashKey("xm_prop_x17_l_glass_03"),
					entityHash = GetHashKey("xm_prop_x17_l_frame_03"),
					entityPos = { x = 371.194, y = 4841.27, z = -60.0 },
				},
			},
			Enable = function(state)
				for key, glass in pairs(DoomsdayFacility.PrivacyGlass.Lounge.Glasses) do
					...
					if state then
						if handle == 0 then
							local entityToAttach = GetClosestObjectOfType(
								glass.entityPos.x,
								glass.entityPos.y,
								glass.entityPos.z,
								1.0,
							...
							if entityToAttach ~= 0 then
								...
								local privacyGlass =
									CreateObject(glass.modelHash, glass.entityPos.x, glass.entityPos.y, glass.entityPos.z, false, false, false)
			...
			Control = {
				...
				Enable = function(state)
					local handle = GetClosestObjectOfType(
						DoomsdayFacility.PrivacyGlass.Lounge.Control.position.x,
						DoomsdayFacility.PrivacyGlass.Lounge.Control.position.y,
						DoomsdayFacility.PrivacyGlass.Lounge.Control.position.z,
						1.0,
						DoomsdayFacility.PrivacyGlass.controlModelHash,
						false,
						false,
					...
					if state then
						if handle == 0 then
							...
							SetEntityRotation(
								privacyGlass,
								DoomsdayFacility.PrivacyGlass.Lounge.Control.rotation.x,
								DoomsdayFacility.PrivacyGlass.Lounge.Control.rotation.y,
								DoomsdayFacility.PrivacyGlass.Lounge.Control.rotation.z,
								2,
								true
							)
							FreezeEntityPosition(privacyGlass, true)
