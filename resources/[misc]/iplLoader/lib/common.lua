...
Global = {
	...
	ResetInteriorVariables = function()
		for key, variable in pairs(Global.Biker) do
			variable = false
		end
		for key, variable in pairs(Global.FinanceOffices) do
			variable = false
		end
		for key, variable in pairs(Global.HighLife) do
			variable = false
		end
	end,
}
...
function GetPedheadshotTexture(ped)
	...
	if pedheadshot ~= -1 then
		...
		if not IsTextureDictLoaded then
			Citizen.Trace(
				'ERROR: BikerClubhouseDrawMembers - Textures dictionnary "' .. tostring(textureDict) .. '" cannot be loaded.'
			)
		end
	else
