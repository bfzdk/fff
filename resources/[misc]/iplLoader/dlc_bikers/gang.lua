			...
			Set = function(member, ped)
				...
				if member.pedheadshot ~= -1 then
					...
					if not IsTextureDictLoaded then
						Citizen.Trace(
							'ERROR: BikerClubhouseDrawMembers - Textures dictionnary "'
								.. tostring(member.textureDict)
								.. '" cannot be loaded.'
						)
					end
				else
...
function DrawClubName(name, color, font)
	if BikerGang.Clubhouse.ClubName.stage == 0 then
		if BikerGang.Clubhouse.ClubName.renderId == -1 then
			BikerGang.Clubhouse.ClubName.renderId =
				CreateNamedRenderTargetForModel(BikerGang.Clubhouse.ClubName.target, BikerGang.Clubhouse.ClubName.prop)
		end
...
function DrawEmblem(texturesDict, rotation)
	if BikerGang.Clubhouse.Emblem.stage == 0 then
		if BikerGang.Clubhouse.Emblem.renderId == -1 then
			BikerGang.Clubhouse.Emblem.renderId =
				CreateNamedRenderTargetForModel(BikerGang.Clubhouse.Emblem.target, BikerGang.Clubhouse.Emblem.prop)
		end
...
function DrawMissions()
	if BikerGang.Clubhouse.MissionsWall.stage == 0 then
		if BikerGang.Clubhouse.MissionsWall.renderId == -1 then
			BikerGang.Clubhouse.MissionsWall.renderId =
				CreateNamedRenderTargetForModel(BikerGang.Clubhouse.MissionsWall.target, BikerGang.Clubhouse.MissionsWall.prop)
		end
