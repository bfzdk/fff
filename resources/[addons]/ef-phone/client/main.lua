...
local PhoneWait = false
local hcfg = lib.callback("ef-phone:fetchhcfg")
local Apartments = hcfg.homes
...
local function CalculateTimeToDisplay()
	-- Hent den aktuelle tid fra systemet
	local currentTime, dateTable = lib.callback.await("ef-phone:fetchOSData")
...
local function DisableDisplayControlActions()
	if PhoneWait then
		...
		DisableControlAction(0, 177, true) -- disable backspace (escape)
		DisableControlAction(0, 200, true) -- disable escape
		DisableControlAction(0, 202, true) -- disable backspace (escape)
...
local function LoadPhone()
	Wait(100)
	lib.callback("ef-phone:fetchMetaData", false, function(MetData)
		lib.callback("qb-phone:server:GetPhoneData", false, function(pData)
			PlayerJob = GetJob()
			PhoneData.PlayerData = GetPlayerData()
			local PhoneMeta = PhoneData.phone or {}
...
local function OpenPhone()
	lib.callback("qb-phone:server:HasPhone", false, function(HasPhone)
		if HasPhone then
			PhoneData.PlayerData = GetPlayerData()
			...
			PhoneData.GarageVehicles = lib.callback.await("qb-garages:server:GetPlayerVehicles")
		else
			Notify("Du har ikke en telefon", "error")
		end
	end)
end
...
local function AnswerCall()
	if
		(PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing")
		and PhoneData.CallData.InCall
		and not PhoneData.CallData.AnsweredCall
	then
...
RegisterCommand("phone", function()
	...
	if not PhoneData.isOpen then
		if not vRP.isHandcuffed() and not IsEntityDead(ped) and not IsPauseMenuActive() then
			OpenPhone()
		else
			Notify("Action not available at the moment..", "error")
		end
	end
end)
...
RegisterNUICallback("GetProfilePicture", function(data, cb)
	...
	lib.callback("qb-phone:server:GetPicture", false, function(picture)
		cb(picture)
	end, number)
end)
...
RegisterNUICallback("PayInvoice", function(data, cb)
	...
	lib.callback("qb-phone:server:PayInvoice", false, function(CanPay, Invoices)
		if CanPay then
...
RegisterNUICallback("DeclineInvoice", function(data, cb)
	...
	local amount = data.amount
	local invoiceId = data.invoiceId
	lib.callback("qb-phone:server:DeclineInvoice", false, function(_, Invoices)
		PhoneData.Invoices = Invoices
		cb("ok")
...
RegisterNUICallback("GetTruckerData", function(_, cb)
	local TruckerMeta = 0 --GetPlayerData().metadata['jobrep']['trucker']
	local TierData = 0 --exports['qb-trucker']:GetTier(TruckerMeta)
	cb(TierData)
end)
...
RegisterNUICallback("track-vehicle", function(data, cb)
	...
	if findVehFromPlateAndLocate(veh.plate) then
		Notify("Dit køretøj er blevet markeret", "success")
	else
		Notify("Dette køretøj kan ikke lokaliseres", "error")
	end
	cb("ok")
end)
...
RegisterNUICallback("GetCryptoData", function(data, cb)
	lib.callback("qb-crypto:server:GetCryptoData", false, function(CryptoData)
		cb(CryptoData)
	end, data.crypto)
end)

RegisterNUICallback("BuyCrypto", function(data, cb)
	lib.callback("qb-crypto:server:BuyCrypto", false, function(CryptoData)
		cb(CryptoData)
	end, data)
end)

RegisterNUICallback("SellCrypto", function(data, cb)
	lib.callback("qb-crypto:server:SellCrypto", false, function(CryptoData)
		cb(CryptoData)
	end, data)
end)

RegisterNUICallback("TransferCrypto", function(data, cb)
	lib.callback("qb-crypto:server:TransferCrypto", false, function(CryptoData)
		cb(CryptoData)
	end, data)
end)
...
RegisterNUICallback("GetAvailableRaces", function(_, cb)
	lib.callback("qb-lapraces:server:GetRaces", false, function(Races)
		cb(Races)
	end)
end)

RegisterNUICallback("ShareContact", function(_, cb)
...
RegisterNUICallback("SetAlertWaypoint", function(data, cb)
	...
	Notify("GPS Location set: " .. data.alert.title)
...
RegisterNUICallback("FetchVehicleResults", function(data, cb)
	lib.callback("qb-phone:server:GetVehicleSearchResults", false, function(result)
		if result ~= nil then
			for k, _ in pairs(result) do
				--lib.callback('police:IsPlateFlagged', false, function(flagged)
...
RegisterNUICallback("FetchVehicleScan", function(_, cb)
	...
	--print('Debug: plate: '..plate..' vehname: '..vehname)

	lib.callback("qb-phone:server:ScanPlate", false, function(result)
		result.isFlagged = flagged
		if QBShared.Vehicles[vehname] ~= nil then
			result.label = QBShared.Vehicles[vehname]["name"]
		else
...
RegisterNUICallback("GetRaces", function(_, cb)
	lib.callback("qb-lapraces:server:GetListedRaces", false, function(Races)
		cb(Races)
	end)
end)

RegisterNUICallback("GetTrackData", function(data, cb)
	lib.callback("qb-lapraces:server:GetTrackData", false, function(TrackData, CreatorData)
...
RegisterNUICallback("HasCreatedRace", function(_, cb)
	lib.callback("qb-lapraces:server:HasCreatedRace", false, function(HasCreated)
		cb(HasCreated)
	end)
end)
...
RegisterNUICallback("IsAuthorizedToCreateRaces", function(data, cb)
	lib.callback("qb-lapraces:server:IsAuthorizedToCreateRaces", false, function(IsAuthorized, NameAvailable)
		data = {
...
RegisterNUICallback("GetRacingLeaderboards", function(_, cb)
	lib.callback("qb-lapraces:server:GetRacingLeaderboards", false, function(Races)
		cb(Races)
	end)
end)

RegisterNUICallback("RaceDistanceCheck", function(data, cb)
	lib.callback("qb-lapraces:server:GetRacingData", false, function(RaceData)
		local ped = PlayerPedId()
		...
		if dist <= 115.0 then
			...
			Notify("You're too far away from the race. GPS has been set to the race.", "error", 5000)
...
RegisterNUICallback("CanRaceSetup", function(_, cb)
	lib.callback("qb-lapraces:server:CanRaceSetup", false, function(CanSetup)
		cb(CanSetup)
	end)
end)

RegisterNUICallback("GetPlayerHouses", function(_, cb)
	lib.callback("qb-phone:server:GetPlayerHouses", false, function(Houses)
		cb(Houses)
	end)
end)
...
RegisterNUICallback("SetHouseLocation", function(data, cb)
	...
	Notify("GPS er blevet sat til " .. data.HouseData.HouseData.adress .. "!", "success")
	cb("ok")
end)
...
RegisterNUICallback("TransferCid", function(data, cb)
	...
	lib.callback("qb-phone:server:TransferCid", false, function(CanTransfer)
		cb(CanTransfer)
	end, TransferedCid, data.HouseData)
end)

RegisterNUICallback("FetchPlayerHouses", function(data, cb)
	lib.callback("qb-phone:server:MeosGetPlayerHouses", false, function(result)
		cb(result)
	end, data.input)
end)
...
RegisterNUICallback("SetGPSLocation", function(data, cb)
	...
	Notify("GPS er blevet sat!", "success")
	cb("ok")
end)

RegisterNUICallback("SetApartmentLocation", function(data, cb)
	local ApartmentData = data.data.appartmentdata
	local TypeData = Apartments[ApartmentData.type]
	SetNewWaypoint(TypeData.entry_point[1], TypeData.entry_point[1])
	Notify("GPS er blevet sat!", "success")
	cb("ok")
end)

RegisterNUICallback("GetCurrentLawyers", function(_, cb)
	lib.callback("qb-phone:server:GetCurrentLawyers", false, function(lawyers)
		cb(lawyers)
	end)
end)
...
RegisterNUICallback("GetWhatsappChats", function(_, cb)
	lib.callback("qb-phone:server:GetContactPictures", false, function(Chats)
		cb(Chats)
	end, PhoneData.Chats)
end)

RegisterNUICallback("CallContact", function(data, cb)
	lib.callback("qb-phone:server:GetCallState", false, function(CanCall, IsOnline, _)
		local status = {
			CanCall = CanCall,
			IsOnline = IsOnline,
...
RegisterNUICallback("SendMessage", function(data, cb)
	...
	local Chat =
		lib.callback.await("qb-phone:server:GetContactPicture", false, PhoneData.Chats[GetKeyByNumber(ChatNumber)])
...
function setupScaleform(scaleform)
	...
	PushScaleformMovieFunction(scaleform, "INPUT_CELLPHONE_SELECT")
	PushScaleformMovieFunctionParameterInt(0)
	Button(GetControlInstructionalButton(2, 176, true))
	ButtonMessage("Tag billede")
	PopScaleformMovieFunctionVoid()

	PushScaleformMovieFunction(scaleform, "INPUT_CELLPHONE_CANCEL")
	PushScaleformMovieFunctionParameterInt(1)
	Button(GetControlInstructionalButton(2, 177, true)) -- The button to display
	ButtonMessage("Annuler") -- the message to display next to it
	PopScaleformMovieFunctionVoid()

	PushScaleformMovieFunction(scaleform, "INPUT_PHONE")
	PushScaleformMovieFunctionParameterInt(2)
	Button(GetControlInstructionalButton(2, 27, true))
	ButtonMessage("Selfie")
	PopScaleformMovieFunctionVoid()
...
RegisterNUICallback("TakePhoto", function(_, cb)
	...
	lib.showTextUI("[PILOP] - Selfie | [ENTER] - Tag billede | [ESC] - Fortryd", {
...
RegisterCommand("ping", function(_, args)
	if not args[1] then
		Notify("You need to input a Player ID", "error")
	else
...
RegisterNetEvent("movebitch", function()
	LoadPhone()
end)
...
RegisterNetEvent("QBCore:Client:OnJobUpdate", function(JobInfo)
	--print('Burde update applications: JobInfo: '..json.encode(JobInfo))
	SendNUIMessage({
		...
		if PhoneData.isOpen then
			if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
				...
				SendNUIMessage({
					...
					PhoneNotify = {
						title = "Besked",
						text = "Sendt besked til dig selv",
						icon = "fab fa-whatsapp",
						color = "#25D366",
						timeout = 4000,
					},
...
RegisterNetEvent("qb-phone:client:addPoliceAlert", function(alertData)
	PlayerJob = GetJob()
	if PlayerJob.name == "Politi-Job" and PlayerJob.onduty then
		SendNUIMessage({
...
RegisterNetEvent("ef-phone:Notify", function(msg, type)
	Notify(msg, type)
end)

function GetJob()
	local job = lib.callback.await("ef-phone:fetchJob")
...
function GetPlayerData()
	local bank, wallet = lib.callback.await("ef-phone:fetchMoney")
	local phone, firstname, name, account, cid = lib.callback.await("ef-phone:fetchCharInfo")
