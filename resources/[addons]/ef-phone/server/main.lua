	...
	if user_id then
		...
		if mailData.button == nil then
			MySQL.insert(
				...
				{ user_id, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0 }
			)
			...
			MySQL.insert(
				...
				{ user_id, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0, json.encode(mailData.button) }
			)
			TriggerClientEvent("qb-phone:client:NewMailNotify", src, mailData)
		end
...
lib.callback.register("qb-phone:server:GetPhoneData", function(src)
	...
	if user_id ~= nil then
		...
		local Tweets =
			MySQL.query.await("SELECT * FROM phone_tweets WHERE `date` > NOW() - INTERVAL ? hour", { Config.TweetDuration })
		...
		local transactions =
			MySQL.query.await("SELECT * FROM crypto_transactions WHERE citizenid = ? ORDER BY `date` ASC", { user_id })
...
lib.callback.register("qb-phone:server:GetVehicleSearchResults", function(_, search)
	...
	local query = "%" .. search .. "%"
	local result =
		MySQL.query.await("SELECT * FROM vrp_user_vehicles WHERE vehicle_plate LIKE ? OR user_id = ?", { query, search })
	if result[1] ~= nil then
		...
		if result[1] ~= nil then
			...
			if Reciever ~= nil then
				...
				MySQL.update(
					...
					{ vRP.getBankMoney({ result[1].user_id }) + amount, result[1].user_id }
				)
			end
...
lib.callback.register("qb-phone:server:GetCurrentLawyers", function()
	...
	for _, src in pairs(vRP.getUsers({})) do
		...
		if Player ~= nil then
			if
				PlayerJob == "Advokat"
				or PlayerJob == "Ejendomsmægler"
				or PlayerJob == "Mekaniker"
				or PlayerJob == "Taxi"
				or PlayerJob == "Politi-Job"
				or PlayerJob == "EMS"
			then
				Lawyers[#Lawyers + 1] = {
...
RegisterNetEvent("qb-phone:server:sendNewMail", function(mailData)
	...
	if mailData.button == nil then
		MySQL.insert(
			"INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)",
			{ Player, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0 }
		)
	else
		MySQL.insert(
			"INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`, `button`) VALUES (?, ?, ?, ?, ?, ?, ?)",
			{ Player, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0, json.encode(mailData.button) }
		)
	end
	...
	for _, src in pairs(vRP.getUsers({})) do
		...
		if Player ~= nil then
			if i.firstname == firstName and i.name == lastName then
				...
				local result =
					MySQL.query.await("SELECT * FROM vrp_user_identities WHERE firstname LIKE ? AND name LIKE ?", { query1, query2 })
...
RegisterNetEvent("qb-phone:server:UpdateTweets", function(NewTweets, TweetData)
	...
	if Config.Linux then
		MySQL.insert(
			"INSERT INTO phone_tweets (citizenid, firstName, lastName, message, date, url, picture, tweetid) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
			{
				TweetData.citizenid,
				TweetData.firstName,
				TweetData.lastName,
...
RegisterNetEvent("qb-phone:server:TransferMoney", function(iban, amount)
	...
	if result[1] ~= nil then
		...
		if reciever ~= nil then
			...
			MySQL.update(
				"UPDATE vrp_user_moneys SET bank = ? WHERE user_id = ?",
				{ RecieverMoney[1].bank + amount, result[1].user_id }
			)
			vRP.tryBankPayment({ sender, amount })
		end
	else
...
RegisterNetEvent("qb-phone:server:RemoveContact", function(Name, Number)
	local src = source
	local Player = vRP.getUserId({ src })
	MySQL.query("DELETE FROM player_contacts WHERE name = ? AND number = ? AND citizenid = ?", { Name, Number, Player })
end)
...
RegisterNetEvent("qb-phone:server:AddNewContact", function(name, number, iban)
	...
	MySQL.insert(
		...
		{ Player, tostring(name), tostring(number), tostring(iban) }
	)
end)
...
RegisterNetEvent("qb-phone:server:UpdateMessages", function(ChatMessages, ChatNumber, _)
	...
	if Player[1] ~= nil then
		...
		if TargetSource ~= nil then
			local Chat =
				MySQL.Sync.fetchAll("SELECT * FROM phone_messages WHERE citizenid = ? AND number = ?", { SenderData, ChatNumber })
			if Chat[1] ~= nil then
				-- Update for target
				MySQL.update(
					"UPDATE phone_messages SET messages = ? WHERE citizenid = ? AND number = ?",
					{ json.encode(ChatMessages), TargetData, i.phone }
				)
				-- Update for sender
				MySQL.update(
					"UPDATE phone_messages SET messages = ? WHERE citizenid = ? AND number = ?",
					{ json.encode(ChatMessages), SenderData, tI.phone }
				)
				-- Send notification & Update messages for target
				TriggerClientEvent(
					"qb-phone:client:UpdateMessages",
					vRP.getUserSource({ Player[1].user_id }),
					ChatMessages,
					i.phone,
					false
				)
				...
				MySQL.insert(
					...
					{ TargetData, i.phone, json.encode(ChatMessages) }
				)
				-- Insert for sender
				MySQL.insert(
					"INSERT INTO phone_messages (citizenid, number, messages) VALUES (?, ?, ?)",
					{ SenderData, tI.phone, json.encode(ChatMessages) }
				)
				-- Send notification & Update messages for target
				TriggerClientEvent(
					"qb-phone:client:UpdateMessages",
			...
			if Chat[1] ~= nil then
				...
				MySQL.update(
					"UPDATE phone_messages SET messages = ? WHERE citizenid = ? AND number = ?",
					{ json.encode(ChatMessages), Player[1].user_id, i.phone }
				)
				-- Update for sender
				Player[1].charinfo = json.decode(Player[1].charinfo)
				MySQL.update(
					"UPDATE phone_messages SET messages = ? WHERE citizenid = ? AND number = ?",
					{ json.encode(ChatMessages), SenderData, Player[1] .. phone }
				)
			else
				-- Insert for target
				MySQL.insert(
					"INSERT INTO phone_messages (citizenid, number, messages) VALUES (?, ?, ?)",
					{ Player[1].citizenid, i.phone, json.encode(ChatMessages) }
				)
				-- Insert for sender
				Player[1].charinfo = json.decode(Player[1].charinfo)
				MySQL.insert(
					"INSERT INTO phone_messages (citizenid, number, messages) VALUES (?, ?, ?)",
					{ SenderData, Player[1].phone, json.encode(ChatMessages) }
				)
			end
		end
	end
...
lib.callback.register("qb-phone:server:TransferCid", function(src, NewCid, house)
	...
	if result[1] then
		...
		success, err = pcall(function()
			updateResult = exports.oxmysql:executeSync(
				"UPDATE vrp_user_homes SET user_id = ? WHERE home = ? AND user_id = ?",
				{ NewCid, house.name, user_id }
			)
		end)
