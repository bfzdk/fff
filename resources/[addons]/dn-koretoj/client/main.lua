...
function limiter(vit)
	...
	if vit == 0 then
		...
		TriggerEvent(
			...
			{
				text = "Fartbegrænser deaktiveret",
				type = "success",
				timeout = 3000,
				layout = "bottomRight",
				queue = "global",
		...
		TriggerEvent(
			"pNotify:SendNotification",
			{
				text = "Fartbegrænser aktiveret til | <b style='color: #4E9350'>" .. vit .. "</b> KM/T",
				type = "success",
				timeout = 3000,
				layout = "bottomRight",
				queue = "global",
...
function toggleEngine(bool)
	...
	if veh ~= 0 then
		...
		TriggerEvent(
			"pNotify:SendNotification",
			{
				text = "Du skal sidde i et køretøj",
				type = "error",
				timeout = 3000,
				layout = "bottomRight",
				queue = "global",
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if veh ~= 0 then
			...
			if vehicleenginestatus == false then
				vehicleenginestatus = true
			end
		end
	end
end)
