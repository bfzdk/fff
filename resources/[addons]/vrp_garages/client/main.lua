...
function vRPgt.spawnGarageVehicle(
	vtype,
	name,
	vehicle_plate,
	vehicle_colorprimary,
	vehicle_colorsecondary,
	vehicle_pearlescentcolor,
	vehicle_wheelcolor,
	vehicle_plateindex,
	vehicle_neoncolor1,
	vehicle_neoncolor2,
...
function StoreVehicle()
	Citizen.CreateThread(function()
		...
		if DoesEntityExist(caissei) then
			...
			local windows =
				AreAllVehicleWindowsIntact(GetClosestVehicle(garageSelected.x, garageSelected.y, garageSelected.z, 3.000, 0, 70))
			if damage < 850 or windows == false then
				TriggerEvent(
					"pNotify:SendNotification",
					{
						text = "Dit køretøj er skadet, ring til en mekaniker!",
						type = "info",
						timeout = 2000,
						layout = "bottomCenter",
						queue = "global",
	...
	while true do
		...
		for _, garage in pairs(Config.Garages) do
			if notgenerating then
				...
				if
					GetDistanceBetweenCoords(garage.x, garage.y, garage.z, GetEntityCoords(LocalPed())) < 3
					and IsPedInAnyVehicle(LocalPed(), true) == false
				then
...
Citizen.CreateThread(function()
	while notparking do
		...
		for _, garage in pairs(Config.Garages) do
			...
			if
				GetDistanceBetweenCoords(garage.x, garage.y, garage.z, GetEntityCoords(LocalPed())) < 3
				and IsPedSittingInAnyVehicle(GetPlayerPed(-1))
			then
...
AddEventHandler("ply_garages:addAptGarage", function(gx, gy, gz, gh)
	...
	if not alreadyExists then
		table.insert(
			garages,
			#garages + 1,
			{ name = "Apartment Garage", colour = 3, id = 357, x = gx, y = gy, z = gz, h = gh }
		)
	end
end)
