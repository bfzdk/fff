...
AddEventHandler("kian-indburd:givback", function()
	...
	if user_id ~= nil then
		...
		PerformHttpRequest(
			...
			"POST",
			json.encode({ username = "Kian - houserobbery", content = "```ID: " .. user_id .. " fik sin lockpick tilbage```" }),
...
AddEventHandler("kian-indburd:lockpickbroke", function()
	...
	if user_id ~= nil then
		...
		PerformHttpRequest(
			...
			function(err, text, headers) end,
			"POST",
			json.encode({ username = "Kian - houserobbery", content = "```ID: " .. user_id .. " fejlede et indbrud.```" }),
			{ ["Content-Type"] = "application/json" }
		)
	end
end)
...
AddEventHandler("kian-indburd:searchItem", function(antal)
	...
	if antal == 0 then
		...
		PerformHttpRequest(
			...
			function(err, text, headers) end,
			"POST",
			json.encode({ username = "PrimeRP - houserobbery", content = "" .. message .. "" }),
			{ ["Content-Type"] = "application/json" }
		)
	else
		...
		PerformHttpRequest(
			...
			"POST",
			json.encode({
				username = "Server Navn - Logs",
				content = "ID: "
					.. user_id
					.. " forsøgte at misbruge indbrud med "
					.. antal
					.. " via "
