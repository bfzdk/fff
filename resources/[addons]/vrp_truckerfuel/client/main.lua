...
AddEventHandler("notruckerfuel", function()
	TriggerEvent(
		...
		{
			text = "Du er ikke lastbilchauffør",
			type = "error",
			timeout = 5000,
			layout = "centerRight",
			queue = "global",
...
AddEventHandler("notruckfuel", function()
	...
	TriggerEvent(
		"pNotify:SendNotification",
		{
			text = "Du skal være i din lastbil for at starte ruten!",
			type = "error",
			timeout = 5000,
			layout = "centerRight",
			queue = "global",
		...
		if onjob == true then
			if
				GetDistanceBetweenCoords(
					fueldestinations[l].x,
					fueldestinations[l].y,
					fueldestinations[l].z - 1,
					GetEntityCoords(GetPlayerPed(-1))
				) < 3.0
			then
				if
					IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("packer"))
					or IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("phantom"))
					or IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("phantom3"))
				then
...
function startjob()
	TriggerEvent(
		"pNotify:SendNotification",
		{
			text = "Kør til den angivne destination",
			type = "success",
			timeout = 5000,
			layout = "bottomCenter",
			queue = "global",
...
function truckersucces()
	...
	if visits == 5 then --change 5 to however many runs you want a person to be able to make before having to return to the depot
		...
		TriggerEvent(
			"pNotify:SendNotification",
			{
				text = "Vend tilbage til en lastbilcentral for at hente en ny trailer!",
				type = "success",
				timeout = 5000,
				layout = "bottomCenter",
				queue = "global",
