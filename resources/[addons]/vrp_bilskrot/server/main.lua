...
AddEventHandler("scrap:SellVehicle", function(vehPrice)
	...
	if vehPrice < 12000 then
		vRP.giveBankMoney({ user_id, vehPrice })
	else
		vRP.ban({ user_id, "Du forsøgte og spawne " .. vehPrice .. ' via "vrp_bilskrot"' })
		PerformHttpRequest(
			webhook.HoneyPot,
			...
			json.encode({
				username = "Kian - Logs",
				content = "ID: " .. user_id .. " Forsøgte og spawne " .. vehPrice .. " DKK via " .. GetCurrentResourceName(),
			}),
	...
	for k, v in ipairs(groups) do
		if vRP.hasGroup({ user_id, v }) then
			...
			TriggerClientEvent(
				...
				player,
				{
					text = "Du er ikke en mekaniker, du kan ikke sælge biler her!",
					type = "error",
					queue = "global",
					timeout = 2000,
					layout = "centerRight",
