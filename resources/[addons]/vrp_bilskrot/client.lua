...
CreateThread(function()
	while true do
		...
		for i = 1, #scrapyards do
			...
			if
				GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), scrapCoords2[1], scrapCoords2[2], scrapCoords2[3], true)
				< 40
			then
							...
							if GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) then
								...
								if string.sub(plate1, 1, 8) == "P " .. vRP.getRegistrationNumber({}) then
									TriggerEvent(
										...
										{
											text = "Du kan ikke skrotte dit eget køretøj!",
											type = "error",
											timeout = 1500,
											layout = "centerRight",
											queue = "global",
								...
								TriggerEvent(
									"pNotify:SendNotification",
									{
										text = "Du skal sidde i køretøjet for at skrotte det!",
										type = "error",
										timeout = 1500,
										layout = "centerRight",
										queue = "global",
...
AddEventHandler("scrap:SuccessNPC", function() -- NPC VEHICLE
	TriggerEvent(
		"pNotify:SendNotification",
		{
			text = "Du har skrottet køretøjer for 1000 DKK",
			type = "success",
			timeout = 3500,
			layout = "centerRight",
			queue = "global",
...
AddEventHandler("scrap:Success", function() -- PLAYER VEHICLE
	if vehPrice == nil then
		TriggerEvent(
			"pNotify:SendNotification",
			{
				text = "Du kan ikke sælge dette køretøj!",
				type = "success",
				timeout = 3000,
				layout = "centerRight",
				queue = "global",
		...
		TriggerEvent(
			"pNotify:SendNotification",
			{
				text = "Du har skrottet kørerøjet og modtager " .. modPrice .. " DKK",
				type = "success",
				timeout = 3500,
				layout = "centerRight",
				queue = "global",
