...
function transfer(fPlayer, tPlayer, amount)
	...
	TriggerClientEvent(
		...
		source,
		{
			text = "Du overførte <b style='color: #4E9350'>" .. math.floor(amount) .. " DKK</b>",
			type = "success",
			queue = "global",
			timeout = 5000,
			layout = "bottomCenter",
	...
	TriggerClientEvent(
		...
		source,
		{
			text = "Nyt indestående beløb: <b style='color: #4E9350'>" .. math.floor(new_balance) .. " DKK</b>",
			type = "info",
			queue = "global",
			timeout = 5000,
			layout = "bottomCenter",
	...
	TriggerClientEvent(
		...
		source2,
		{
			text = "Du modtog <b style='color: #4E9350'>" .. math.floor(amount)(" DKK</b>"),
			type = "success",
			queue = "global",
			timeout = 5000,
			layout = "bottomCenter",
	...
	TriggerClientEvent(
		...
		source2,
		{
			text = "Nyt indestående beløb: <b style='color: #4E9350'>" .. math.floor(new_balance2)(" DKK</b>"),
			type = "success",
			queue = "global",
			timeout = 5000,
			layout = "bottomCenter",
...
AddEventHandler("bank:depositAll", function()
	...
	if wallet > 0 then
		...
		TriggerClientEvent(
			...
			source,
			{
				text = "Du har ingen kontanter på dig!",
				type = "error",
				queue = "global",
				timeout = 5000,
				layout = "bottomCenter",
...
AddEventHandler("bank:deposit", function(amount)
	...
	if rounded <= wallet then
		...
		TriggerClientEvent(
			...
			source,
			{
				text = "Du har ikke nok kontanter",
				type = "error",
				queue = "global",
				timeout = 5000,
				layout = "bottomCenter",
...
AddEventHandler("bank:withdraw", function(amount)
	...
	if tonumber(rounded) <= tonumber(bankbalance) then
		...
		TriggerClientEvent(
			...
			user_id,
			{
				text = "Du har ikke nok penge på kontoen",
				type = "error",
				queue = "global",
				timeout = 5000,
				layout = "bottomCenter",
...
AddEventHandler("bank:transfer", function(toPlayer, amount)
	...
	if tonumber(user_id) == tonumber(toPlayer) then
		TriggerClientEvent(
			...
			source,
			{
				text = "Du kan ikke overføre til dig selv",
				type = "error",
				queue = "global",
				timeout = 5000,
				layout = "bottomCenter",
