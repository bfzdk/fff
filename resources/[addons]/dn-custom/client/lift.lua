			...
			if lift ~= 0 then
				...
				if Vdist(coords["x"], coords["y"], coords["z"], lcoords["x"] - 2.5, lcoords["y"], lcoords["z"]) < 1.2 then
					...
					if IsControlJustPressed(1, Keys["E"]) then
						if usedlift[math.floor(lcoords["x"])] == nil then
							local veh =
								GetClosestVehicle(lcoords["x"] + 0.0001, lcoords["y"] + 0.0001, lcoords["z"] + 0.0001, 1 + 0.0001, 0, 4 + 2 + 1)
							if veh ~= 0 then
								...
								TriggerEvent(
									...
									{ text = "Der skal være et køretøj!", type = "error", timeout = 3000, layout = "centerRight" }
								)
							end
						else
