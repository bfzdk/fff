...
Citizen.CreateThread(function()
	while true do
		...
		if
			GetDistanceBetweenCoords(objectcoords.x, objectcoords.y, objectcoords.z, getPlayerCoords) < 1.5
			and not using
		...
		then
			if isBed == true then
				...
				DrawText3Ds(
					...
					objectcoords.y,
					objectcoords.z + 0.20,
					"~w~ Skift mellem maven, ryggen og sidde med ~g~pile tasterne"
				)
...
function Execute(object, vert, verty, dir, isBed, ped, objectcoords)
	...
	if isBed == false then
		...
		TaskStartScenarioAtPosition(
			...
			objectcoords.z - verticalOffset,
			GetEntityHeading(object) + direction,
			0,
			true,
			true
		)
