...
Citizen.CreateThread(function()
	...
	while true do
		...
		local rayHandle =
			CastRayPointToPoint(posped.x, posped.y, posped.z, entityWorld.x, entityWorld.y, entityWorld.z, 10, ped, 0)
...
Citizen.CreateThread(function()
	while true do
		...
		if Vehicle.Vehicle ~= nil then
			if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= 200 then
				DisplayHelpText("Tryk [~g~SHIFT~w~] og [~g~E~w~] for at skubbe køretøjet!")
			end
			if
				IsControlPressed(0, Keys["LEFTSHIFT"])
				and IsVehicleSeatFree(Vehicle.Vehicle, -1)
				and not IsEntityAttachedToEntity(ped, Vehicle.Vehicle)
				and IsControlJustPressed(0, Keys["E"])
				and GetVehicleEngineHealth(Vehicle.Vehicle) <= 200
			then
