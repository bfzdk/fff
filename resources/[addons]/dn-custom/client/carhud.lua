...
local fuelWarnLimit = 25 -- Fuel limit for triggering warning color
...
-- SEATBELT PARAMETERS
local seatbeltInput = 20 -- Toggle seatbelt on/off with K or DPAD down (controller)
...
local directions =
	{ [0] = "N", [1] = "NV", [2] = "V", [3] = "SV", [4] = "S", [5] = "SØ", [6] = "Ø", [7] = "NØ", [8] = "N" }
local zones = {
	...
	["EBURO Børge Remake"] = "El Burro Heights",
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if pedInVeh or locationAlwaysOn then
			...
			drawTxt(timeText, 4, locationColorText, 0.40, screenPosX, screenPosY + 0.048)
			...
			if pedInVeh and GetIsVehicleEngineRunning(vehicle) and vehicleClass ~= 13 then
				...
				if ShouldUseMetricMeasurements() then
					...
					local speedColor = (speed >= speedLimit) and speedColorOver or speedColorUnder or speedColorMellem
					drawTxt(("%.3d"):format(math.ceil(speed)), 0.4, speedColor, 0.39, screenPosX + 0.001, screenPosY + 0.017)
					drawTxt("KMT", 2, speedColorText, 0.4, screenPosX + 0.029, screenPosY + 0.018)
				else
				...
				--drawTxt("BENZIN", 2, fuelColorText, 0.4, screenPosX + 0.084, screenPosY + 0.018)

				-- Draw cruise control status
				local cruiseColor = cruiseIsOn and cruiseColorOn or cruiseColorOff
				drawTxt("Fartpilot", 2, cruiseColor, 0.4, screenPosX + 0.039, screenPosY + 0.048)

				-- Draw seatbelt status if not a motorcyle
				if vehicleClass ~= 8 then
					local seatbeltColor = seatbeltIsOn and seatbeltColorOn or seatbeltColorOff
					drawTxt("Sele", 2, seatbeltColor, 0.4, screenPosX + 0.084, screenPosY + 0.048) -- GET_VEHICLE_NUMBER_PLATE_TEXT(vehicle);
				end
			end
		end
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if pedInVeh or locationAlwaysOn then
			...
			local hour = GetClockHours() + 3
			local minute = GetClockMinutes()
			timeText = ("%.2d"):format((hour == 0) and 12 or hour)
				.. ":"
				.. ("%.2d"):format(minute)
			...
			locationText = (zoneNameFull == "" or zoneNameFull == nil) and locationText
				or (locationText .. " | " .. zoneNameFull)
