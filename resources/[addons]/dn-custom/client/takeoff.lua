...
function changeCloth(cloth)
	if not vRP.isHandcuffed() then
		...
		if cloths[cloth].status then
			...
			if cloths[cloth].start ~= nil then
				Citizen.Wait(cloths[cloth].start)
			else
				Citizen.Wait(1000)
			end
			if cloths[cloth].comp ~= nil then
				SetPedComponentVariation(
					ped,
					cloths[cloth].comp,
					cloths[cloth].cloth.draw,
					cloths[cloth].cloth.text,
					cloths[cloth].cloth.pal
				)
			...
			if cloths[cloth].stop ~= nil then
				Citizen.Wait(cloths[cloth].stop)
			else
				Citizen.Wait(1000)
			end
			if cloths[cloth].comp ~= nil then
