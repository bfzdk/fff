	...
	if isVehicleTow then
			...
			while true do
				...
				if IsEntityUpsidedown(GetVehiclePedIsIn(PlayerPedId(), true)) and isVehicleTow or roll > 70.0 or roll < -70.0 then
					...
					TriggerEvent(
						...
						{
							text = "Kablerne knækket imens de holdte køretøjet",
							type = "error",
							timeout = 5000,
							layout = "centerRight",
							queue = "global",
			...
			if targetVehicle ~= 0 then
				...
				if distanceBetweenVehicles > 12.0 then
					TriggerEvent(
						"pNotify:SendNotification",
						{
							text = "Kablerne kan ikke række så langt",
							type = "error",
							timeout = 5000,
							layout = "centerRight",
							queue = "global",
					...
					if
						not (
							(not allowTowingBoats and IsThisModelABoat(targetModelHash))
							or (not allowTowingHelicopters and IsThisModelAHeli(targetModelHash))
							or (not allowTowingPlanes and IsThisModelAPlane(targetModelHash))
							or (not allowTowingTrains and IsThisModelATrain(targetModelHash))
							or (not allowTowingTrailers and isTargetVehicleATrailer(targetModelHash))
						)
					then
						if not IsPedInAnyVehicle(playerped, true) then
							if vehicle ~= targetVehicle and IsVehicleStopped(vehicle) then
								...
								TriggerEvent(
									"pNotify:SendNotification",
									{
										text = "Køretøjet blev kørt på ladet",
										type = "success",
										timeout = 5000,
										layout = "centerRight",
										queue = "global",
								...
								TriggerEvent(
									"pNotify:SendNotification",
									{
										text = "Der er ikke noget køretøj på ladet",
										type = "error",
										timeout = 5000,
										layout = "centerRight",
										queue = "global",
						...
						TriggerEvent(
							"pNotify:SendNotification",
							{
								text = "Køretøjet kan ikke køres på ladet",
								type = "error",
								timeout = 5000,
								layout = "centerRight",
								queue = "global",
		...
		elseif IsVehicleStopped(vehicle) then
			...
			TriggerEvent(
				"pNotify:SendNotification",
				{
					text = "Køretøjet blev kørt af ladet",
					type = "success",
					timeout = 5000,
					layout = "centerRight",
					queue = "global",
		...
		TriggerEvent(
			"pNotify:SendNotification",
			{
				text = "Din køretøj er ikke indregisteret som en tow truck",
				type = "error",
				timeout = 5000,
				layout = "centerRight",
				queue = "global",
