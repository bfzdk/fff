		...
		if user_id ~= nil then
			if vRP.hasPermission({ user_id, "player.kick" }) then
					...
					function(source, nuser_id)
						if nuser_id ~= nil and nuser_id ~= "" and tonumber(nuser_id) > 0 and tonumber(nuser_id) then
							...
							TriggerClientEvent(
								...
								nsource,
								{
									text = "<center><h2>Du bedes venligst at sætte dig i afventer support inden for 10 min, ellers Ban</h2></center>",
									type = "error",
									queue = "global",
									timeout = 600000,
									layout = "center",
							...
							TriggerClientEvent(
								...
								source,
								{
									text = "Advarsel sendt [Når denne går væk er der gået 10min]",
									type = "info",
									timeout = 600000,
									layout = "bottomCenter",
									queue = "global",
							...
							local dmessage = "**[ ID: "
								.. tostring(user_id)
								.. " ]** sendte discord advarsle til **[ ID: "
								.. tostring(nuser_id)
								.. " ]**."
							PerformHttpRequest(
								"https://discordapp.com/api/webhooks/644483281550573578/0ugMzpbv7MB2eQkxh0BOXwLuaF-kzmA3WxJc9Tf9voAmky_U_GXSDmFetz2HMkeEfLoA",
				...
				TriggerClientEvent(
					...
					source,
					{
						text = "<center><h2>Du skal være staff for at bruge den her command</h2></center>",
						type = "info",
						timeout = 5000,
						layout = "bottomCenter",
						queue = "global",
