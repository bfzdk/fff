...
if Config.General.CheckForUpdates then
	PerformHttpRequest(
		"https://raw.githubusercontent.com/Wolxy/Wolxy-Garderobe/main/Resource%20Mappe/Wolxy-Garderobe/Version",
		...
		function(Data)
			...
			GarderobeMenu[Config.Menus.Garderobe.Buttons.SaveNewOutfit.Name] = {
				function(Player, Choice)
					vRP.prompt({
						Player,
						"Vælg et navn til dit nye outfit",
						"",
...
function CreateAreas(Source)
	...
	for _, v in pairs(Config.Locations) do
		...
		if Config.Markers.Enabled then
			vRPclient.addMarker(
				Source,
				{ v.X, v.Y, v.Z - 1.0, 1.0, 1.0, 0.5, Config.Markers.R, Config.Markers.G, Config.Markers.B, 255, 200.0 }
			)
		end
...
function NotifyPlayer(Player, Notification)
	...
	elseif System == "t-notify" then
		...
		TriggerClientEvent("t-notify:client:Custom", Player, {
			...
			position = tNotify_FirstLetter[Notification.Location:sub(1, 1)]
				.. tNotify_SecondLetter[Notification.Location:sub(-1)],
		})
