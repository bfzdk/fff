...
function Menu.Option(option)
	...
	elseif optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption then
		GUI.Text(
			option,
			GUI.optionText,
			{ menuX - 0.1, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125 },
			{ 0.5, 0.5 },
			false
		)
		GUI.Rect(
			GUI.optionRect,
			{ menuX, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.1415 },
			{ 0.27, 0.035 }
		)
		if thisOption then
	...
	elseif optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption then
		if bool then
			GUI.Text(
				"~g~ON",
				GUI.optionText,
				{ menuX + 0.068, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125 },
				{ 0.5, 0.5 },
				true
			)
		else
...
function Menu.Int(option, int, min, max, cb)
	...
	elseif optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption then
		GUI.Text(
			tostring(int),
			GUI.optionText,
			{ menuX + 0.068, optionCount - (currentOption - maxVisOptions) * 0.035 + 0.125 },
			{ 0.5, 0.5 },
			true
		)
	end
