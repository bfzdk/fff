...
Citizen.CreateThread(function()
	...
	while true do
		...
		if
			isNearFuelStation
			and IsPedInAnyVehicle(GetPlayerPed(-1), -1)
			and not IsPedInAnyHeli(GetPlayerPed(-1))
			and not isBlackListedModel()
			and not isElectricModel()
			and GetPedVehicleSeat(GetPlayerPed(-1)) == -1
		then
		...
		if
			(isNearFuelStation or isNearFuelPStation or isNearFuelHStation or isNearFuelBStation)
			and not IsPedInAnyVehicle(GetPlayerPed(-1))
		then
			drawTxt("[~g~E~s~] for at købe en dunk (~b~50 DKK~w~)", 0, 1, 0.5, 0.8, 0.6, 255, 255, 255, 255)
...
Citizen.CreateThread(function()
	while true do
		...
		if
			IsPedInAnyVehicle(GetPlayerPed(-1), -1)
			and isEngineOn
			and GetPedVehicleSeat(GetPlayerPed(-1)) == -1
			and not isBlackListedModel()
		then
...
AddEventHandler("essence:hasBuying", function(amount, toPay)
	TriggerEvent(
		...
		{
			text = "Du har købt " .. amount .. " liter brændstof for <b style='color: #DB4646'>" .. toPay .. " DKK</b>",
			type = "success",
			timeout = 4000,
			layout = "centerRight",
			queue = "global",
...
AddEventHandler("advancedFuel:setEssence", function(percent, plate, model)
	...
	if
		GetVehicleNumberPlateText(GetVehiclePedIsUsing(GetPlayerPed(-1))) == plate
		and model == GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsUsing(GetPlayerPed(-1))))
	then
		essence = toEssence
		local essenceToPercent = (essence / 0.142) * 65
		SetVehicleFuelLevel(GetVehiclePedIsIn(GetPlayerPed(-1)), round(essenceToPercent))
...
function IsVehInArray()
	for i, k in pairs(vehiclesUsed) do
		if
			k.plate == GetVehicleNumberPlateText(GetVehiclePedIsUsing(GetPlayerPed(-1)))
			and k.model == GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsUsing(GetPlayerPed(-1))))
		then
			return true
		end
	end
