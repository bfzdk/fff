...
function Menu.PageCounter()
	GUI.Text(
		currentOption .. "/" .. Menus[curMenu].optionCount,
		GUI.optionText,
		{ menuX + menuXOption - 0.02, ((menuYOptionAdd - 0.018) + (optionCount / menuYOptionDiv) * menuYModify) },
		optionTextSize,
		false
	)
	...
	DrawSprite(
		...
		menuX,
		((menuYOptionAdd - 0.018) + (optionCount / menuYOptionDiv) * menuYModify + 0.015),
		0.03,
		0.03,
		0.0,
		255,
	...
	GUI.Rect(
		GUI.titleRect,
		{ menuX, (menuYOptionAdd + (optionCount / menuYOptionDiv) * menuYModify + 0.015) },
		{ optionRectSize[1], optionRectSize[2] - 0.03 }
	)
end
...
function Menu.Option(option)
	...
	if currentOption <= GUI.maxVisOptions and optionCount <= GUI.maxVisOptions then
		GUI.Text(
			option,
			GUI.optionText,
			{ menuX - menuXOption, ((menuYOptionAdd - 0.018) + (optionCount / menuYOptionDiv) * menuYModify) },
			optionTextSize,
			false
		)
		GUI.Rect(GUI.optionRect, { menuX, (menuYOptionAdd + (optionCount / menuYOptionDiv) * menuYModify) }, optionRectSize)
	...
	elseif optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption then
		GUI.Text(
			...
			{
				menuX - menuXOption,
				((menuYOptionAdd - 0.018) + ((optionCount - (currentOption - GUI.maxVisOptions)) / menuYOptionDiv) * menuYModify),
			},
			optionTextSize,
			false
		)
		GUI.Rect(
			GUI.optionRect,
			{ menuX, (menuYOptionAdd + ((optionCount - (currentOption - GUI.maxVisOptions)) / menuYOptionDiv) * menuYModify) },
			optionRectSize
		)
		if thisOption then
			GUI.Rect(
...
function Menu.changeMenu(option, menu)
	...
	if currentOption <= GUI.maxVisOptions and optionCount <= GUI.maxVisOptions then
		GUI.Text(
			">>",
			GUI.optionText,
			{ menuX + menuXOtherOption, ((menuYOptionAdd - 0.018) + (optionCount / menuYOptionDiv) * menuYModify) },
			optionTextSize,
			true
		)
	elseif optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption then
...
function Menu.ScrollBarStringSelect(array, min, cb)
	...
	if optionCount == currentOption then
		...
		if rightPressed then -- right
			if currentPosition < maxPosition - 1 then
				currentPosition = currentPosition + 1
			elseif currentPosition == maxPosition - 1 then
				currentPosition = 0
			else
				currentPosition = maxPosition - 1
			end
	...
	if currentOption <= GUI.maxVisOptions and optionCount <= GUI.maxVisOptions then
		...
		local new_x = (bar.x - ((bar.width.background_box - bar.width.scroller) / 2))
			+ (((bar.width.background_box - bar.width.scroller) / (maxPosition - 1)) * currentPosition)
		if new_x < (bar.x - ((bar.width.background_box - bar.width.scroller) / 2)) then
			new_x = (bar.x - ((bar.width.background_box - bar.width.scroller) / 2))
		end ---- SCROLLER MIN
		if new_x > (bar.x + ((bar.width.background_box - bar.width.scroller) / 2)) then
			new_x = (bar.x + ((bar.width.background_box - bar.width.scroller) / 2))
		end ---- SCROLLER MAX
...
function Menu.ScrollBarString(array, min, cb)
	...
	if optionCount == currentOption and leftPressed then
		cb(currentPosition)
		return true
	elseif optionCount == currentOption and rightPressed then
		cb(currentPosition)
