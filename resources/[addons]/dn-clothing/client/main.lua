...
function customise(title)
	...
	if
		GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01")
		or GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01")
	then
		componentScroller = 0
		...
		Menu.addOption("clothing_customise", function()
			...
			if componentScroller == 2 then
				textureMax = GetNumHairColors()
			else
				textureMax = GetNumberOfPedTextureVariations(GetPlayerPed(-1), componentScroller, subComponentScroller)
			end
		...
		Menu.addOption("clothing_customise", function()
			if Menu.ScrollBarInt("Colour Palette", paletteScroller, 2, function(cb)
				paletteScroller = cb
			end) then
				SetPedComponentVariation(
					GetPlayerPed(-1),
		...
		Menu.addOption("clothing_customise", function()
			local precomponentTable =
				{ "Ansigt", "Ingen", "Hår", "Bluse", "Bukser", "Ingen", "Ingen", "Ingen", "Smykker", "Ingen", "Ingen", "Ingen" }
			local componentTable = {}
			for i = 0, 11 do
				if
					GetNumberOfPedDrawableVariations(GetPlayerPed(-1), i) ~= 0
					and GetNumberOfPedDrawableVariations(GetPlayerPed(-1), i) ~= false
				then
			...
			if Menu.ScrollBarString(componentTable, componentScroller, function(cb)
				componentScroller = cb
			end) then
				subComponentScroller = GetPedDrawableVariation(GetPlayerPed(-1), componentScroller)
				textureScroller = GetPedTextureVariation(GetPlayerPed(-1), componentScroller)
		...
		Menu.addOption("clothing_customise", function()
			if
				Menu.ScrollBarInt(
					"Textures",
					textureScroller,
					GetNumberOfPedTextureVariations(GetPlayerPed(-1), componentScroller, subComponentScroller),
					function(cb)
						textureScroller = cb
					end
				)
...
function accessories(title)
	...
	then
		...
		Menu.addOption("clothing_accessories", function()
			if
				Menu.ScrollBarString(
					{ "Hjelme", "Briller", "Øringe", "Tom Plads", "Tom Plads", "Tom Plads", "Venstre håndled", "Højre håndled" },
					componentScroller,
					function(cb)
						componentScroller = cb
					end
		...
		Menu.addOption("clothing_accessories", function()
			if
				Menu.ScrollBarInt(
					"Components",
					subComponentScroller,
					GetNumberOfPedPropDrawableVariations(GetPlayerPed(-1), componentScroller),
					function(cb)
						subComponentScroller = cb
					end
				)
		...
		Menu.addOption("clothing_accessories", function()
			if
				Menu.ScrollBarInt(
					"Textures",
					textureScroller,
					GetNumberOfPedPropTextureVariations(PlayerPedId(), componentScroller, subComponentScroller),
					function(cb)
						textureScroller = cb
					end
				)
		...
		Menu.addOption("clothing_accessories", function()
			if
				Menu.ScrollBarStringSelect(
					{ "Slet Hjelme", "Slet briller", "Slet Øringe", "Slet Venstre håndled", "Slet Højre håndled" },
					removeScroller,
					function(cb)
						removeScroller = cb
					end
		...
		for i = 0, 7 do
			if
				GetNumberOfPedPropDrawableVariations(GetPlayerPed(-1), i) ~= 0
				and GetNumberOfPedPropDrawableVariations(GetPlayerPed(-1), i) ~= false
			then
				componentTable[i + 1] = precomponentTable[i + 1]
			else
				componentTable[i + 1] = "Empty slot"
...
function overlays(title)
	...
	then
		...
		Menu.addOption("clothing_overlays", function()
			if Menu.ScrollBarInt("Opacity", opacityScroller, 10, function(cb)
				opacityScroller = cb
			end) then
		...
		Menu.addOption("clothing_overlays", function()
			if Menu.ScrollBarInt("Colours", colourScroller, 63, function(cb)
				colourScroller = cb
			end) then
				local colourType = 0
				if componentScroller == 1 or componentScroller == 2 or componentScroller == 10 then
					colourType = 1
				elseif componentScroller == 5 or componentScroller == 8 then
					colourType = 2
				else
					colourType = 0
				end
...
function save()
	...
	if player_data.clothing.drawables[12] == 55 and GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01") then
		player_data.clothing.drawables[12] = 56
		SetPedComponentVariation(GetPlayerPed(-1), 11, 56, 0, 2)
	end
	if player_data.clothing.drawables[12] == 48 and GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01") then
		player_data.clothing.drawables[12] = 49
		SetPedComponentVariation(GetPlayerPed(-1), 11, 49, 0, 2)
	end
	...
	then
		for i = 0, 11 do
			if i == 0 then
				SetPedHeadBlendData(
					GetPlayerPed(-1),
					player_data.clothing.drawables[i + 1],
					player_data.clothing.drawables[i + 1],
					0,
					player_data.clothing.drawables[i + 1],
					player_data.clothing.drawables[i + 1],
			...
			elseif i == 2 then
				...
				SetPedComponentVariation(
					...
					i,
					player_data.clothing.drawables[i + 1],
					player_data.clothing.textures[i + 1],
					player_data.clothing.palette[i + 1]
				)
			end
		end
		for i = 0, 7 do
		...
		for i = 0, 12 do
			...
			SetPedHeadOverlayColor(
				GetPlayerPed(-1),
				i,
				player_data.overlays.colours[i + 1].colourType,
				player_data.overlays.colours[i + 1].colour,
				player_data.overlays.colours[i + 1].colour
			)
		end
	else
