...
AddEventHandler("LSC:finished", function(VehicleModel, plate, veh, prcolor, secolor)
	...
	if user_id then
		MySQL.Async.execute(
			"update vrp_user_vehicles set modifications = @modifications where user_id = @user_id and hashkey = @hashkey and vehicle_plate = @vehicle_plate",
			{
				user_id = user_id,
				hashkey = VehicleModel,
				vehicle_plate = plate,
				modifications = json.encode({
					color = veh.color,
					extraColor = veh.extracolor,
					neon = veh.neon,
					neonColor = veh.neoncolor,
					xenonColor = veh.xenoncolor,
					smokeColor = veh.smokecolor,
					wheelType = veh.wheeltype,
					bulletProofTyres = veh.bulletProofTyres,
					windowTint = veh.windowtint,
					plateIndex = veh.plateindex,
					mods = veh.mods,
				}),
			}
		)
		MySQL.Async.execute(
			"update vrp_user_vehicles set vehicle_colorprimary = @pcolor where user_id = @user_id and hashkey = @hashkey and vehicle_plate = @vehicle_plate",
			{ user_id = user_id, hashkey = VehicleModel, vehicle_plate = plate, pcolor = prcolor }
		)
		MySQL.Async.execute(
			"update vrp_user_vehicles set vehicle_colorsecondary = @scolor where user_id = @user_id and hashkey = @hashkey and vehicle_plate = @vehicle_plate",
			{ user_id = user_id, hashkey = VehicleModel, vehicle_plate = plate, scolor = secolor }
		)
	end
end)
...
AddEventHandler("LSC:applyModifications", function(key, vehicle, plate)
	...
	if user_id then
		MySQL.Async.fetchAll(
			"select modifications from vrp_user_vehicles where user_id = @user_id and hashkey = @hashkey and vehicle_plate = @plate",
			{ user_id = user_id, hashkey = key, plate = plate },
