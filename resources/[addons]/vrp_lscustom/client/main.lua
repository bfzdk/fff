...
local garages = {
	[1] = {
		...
		inside = { x = -337.3863, y = -136.9247, z = 38.5737, heading = 269.455, hidden = false },
	},
	[2] = {
		locked = false,
		...
		inside = { x = 733.69, y = -1088.74, z = 21.733, heading = 270.528, hidden = false },
	},
	[3] = {
		locked = false,
		...
		inside = { x = -1155.077, y = -2006.61, z = 12.465, heading = 162.58, hidden = false },
	},
	[4] = {
		locked = false,
		...
		inside = { x = 1174.823, y = 2637.807, z = 37.045, heading = 181.19, hidden = false },
	},
	[5] = {
		locked = false,
		...
		inside = { x = 108.842, y = 6628.447, z = 31.072, heading = 45.504, hidden = false },
	},
	[6] = {
		locked = false,
		...
		inside = { x = -212.368, y = -1325.486, z = 30.176, heading = 141.107, hidden = false },
	},
	[7] = {
		locked = false,
		camera = { x = 877.441, y = -3246.062, z = -98.285, heading = 88.726 },
		driveout = { x = 873.132, y = -3245.980, z = -98.288, heading = 88.944 },
		drivein = { x = 869.336, y = -3245.908, z = -98.291, heading = 88.961 },
		outside = { x = 865.545, y = -3245.993, z = -98.294, heading = 90.494 },
		inside = { x = 860.964, y = -3246.108, z = -98.294, heading = 90.762, hidden = false },
	},
	[8] = {
		locked = false,
		camera = { x = -1755.092, y = 3049.597, z = 34.454, heading = 143.840 },
		driveout = { x = -1755.461, y = 3056.094, z = 32.825, heading = 326.187 },
		drivein = { x = -1759.247, y = 3058.229, z = 32.823, heading = 187.599 },
		outside = { x = -1752.709, y = 3058.762, z = 32.829, heading = 298.531 },
		inside = { x = -1758.556, y = 3050.991, z = 32.818, heading = 185.213, hidden = true },
	},
}
...
local function DriveInGarage()
	...
	m = LSCMenu.categories.Wheels:addSubMenu(
		"WHEEL ACCESSORIES",
		"Wheel accessories",
			...
			if inside == false then
				...
				if IsPedSittingInAnyVehicle(ped) then
					...
					if
						...
						and (IsThisModelACar(GetEntityModel(veh)) or IsThisModelABike(GetEntityModel(veh)))
					then
						...
						for i, pos in ipairs(garages) do
							...
							if LSC_Config.oldenter then
								...
								if GetDistanceBetweenCoords(outside.x, outside.y, outside.z, GetEntityCoords(ped)) <= f(5) then
									...
									if
										not tableContains(LSC_Config.ModelBlacklist, GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower())
									then
								...
								if
									math.abs(GetEntityHeading(veh) - outside.heading) <= 90
									and IsVehicleStopped(veh)
									and GetDistanceBetweenCoords(outside.x, outside.y, outside.z, GetEntityCoords(ped)) <= f(5)
								then
									--Lets check if our vehicle is not in the model black list, and if it is not then we can go further
									if
										not tableContains(LSC_Config.ModelBlacklist, GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower())
...
AddEventHandler("LSC:buttonSelected", function(name, button, canpurchase)
	...
	if
		...
		or mname == "arch cover"
		or mname == "aerials"
		or mname == "roof scoops"
		or mname == "doors"
		or mname == "roll cage"
		or mname == "engine block"
		or mname == "cam cover"
		or mname == "strut brace"
		or mname == "trim design"
		or mname == "ormnametns"
		or mname == "dashboard"
		or mname == "dials"
		or mname == "seats"
		or mname == "steering wheels"
		or mname == "plate holder"
		or mname == "vanity plates"
		or mname == "shifter leavers"
		or mname == "plaques"
		or mname == "speakers"
		or mname == "trunk"
		or mname == "armor"
		or mname == "suspension"
		or mname == "transmission"
		or mname == "brakes"
		or mname == "engine tunes"
		or mname == "roof"
		or mname == "hood"
		or mname == "grille"
		or mname == "roll cage"
		or mname == "exhausts"
		or mname == "skirts"
		or mname == "rear bumpers"
		or mname == "front bumpers"
		or mname == "spoiler"
	then
		if button.name == "Stock" or button.purchased or CanPurchase(price, canpurchase) then
...
AddEventHandler("LSC:applyModifications", function(vehicle, modifications)
	if vehicle and modifications then
		...
		SetVehicleNeonLightsColour(
			vehicle,
			modifications.neonColor[1],
			modifications.neonColor[2],
			modifications.neonColor[3]
		)
		SetVehicleHeadlightsColour(vehicle, modifications.xenonColor)
		SetVehicleTyreSmokeColor(
			vehicle,
			modifications.smokeColor[1],
			modifications.smokeColor[2],
			modifications.smokeColor[3]
		)
...
function CheckPurchases(m)
	...
	elseif
		...
		or name == "exhausts"
		or name == "skirts"
		or name == "rear bumpers"
		or name == "front bumpers"
		or name == "spoiler"
	then
		for i, b in pairs(m.buttons) do
			if b.mod == -1 then
	...
	elseif name == "neon layout" then
		for i, b in pairs(m.buttons) do
			if b.name == "None" then
				if
					IsVehicleNeonLightEnabled(myveh.vehicle, 0) == false
					and IsVehicleNeonLightEnabled(myveh.vehicle, 1) == false
					and IsVehicleNeonLightEnabled(myveh.vehicle, 2) == false
					and IsVehicleNeonLightEnabled(myveh.vehicle, 3) == false
				then
					b.sprite = "garage"
				else
					b.sprite = nil
			...
			elseif b.name == "Front,Back and Sides" then
				if
					IsVehicleNeonLightEnabled(myveh.vehicle, 0)
					and IsVehicleNeonLightEnabled(myveh.vehicle, 1)
					and IsVehicleNeonLightEnabled(myveh.vehicle, 2)
					and IsVehicleNeonLightEnabled(myveh.vehicle, 3)
				then
					b.sprite = "garage"
				else
					b.sprite = nil
	...
	elseif
		...
		or name == "dials"
		or name == "seats"
		or name == "steering wheels"
		or name == "plate holder"
		or name == "vanity plates"
		or name == "shifter leavers"
		or name == "plaques"
		or name == "speakers"
		or name == "trunk"
		or name == "headlights"
