...
function Menu:draw()
	Citizen.CreateThread(function()
		...
		while self.visible do
			...
			drawRightMenuText(
				menu.selected .. "/" .. tablelength(menu.buttons),
				cfg.font,
				0,
				x + (width / 2.1),
				ty,
			...
			for i, btn in pairs(buttons) do
				if i >= menu.from and i <= menu.to then
					if menu.selected == i then
						...
						if
							IsControlJustReleased(1, self.config.controls.menu_select)
							or IsDisabledControlJustReleased(1, self.config.controls.menu_select)
						then
						...
						if btn.list then
							...
							if
								IsControlJustPressed(1, self.config.controls.menu_left)
								or IsDisabledControlJustPressed(1, self.config.controls.menu_left)
							then
							...
							if
								IsControlJustPressed(1, self.config.controls.menu_right)
								or IsDisabledControlJustPressed(1, self.config.controls.menu_right)
							then
			...
			elseif menu.buttons[menu.selected] and menu.buttons[menu.selected].info ~= nil then
				DrawRect(x, y - height + y - ty + 0.0055, width, 0.0035, cfg.sbg_color.r, cfg.sbg_color.g, cfg.sbg_color.b, 255)
				drawInfo(
					menu.buttons[menu.selected].info,
					cfg.font,
					x,
					y,
					width,
			...
			if
				IsControlJustPressed(1, self.config.controls.menu_up) or IsDisabledControlJustPressed(
					1,
					self.config.controls.menu_up
				)
			then
			...
			if
				IsControlJustPressed(1, self.config.controls.menu_down)
				or IsDisabledControlJustPressed(1, self.config.controls.menu_down)
			then
			...
			if
				IsControlJustReleased(1, self.config.controls.menu_back)
				or IsDisabledControlJustReleased(1, self.config.controls.menu_back)
			then
