...
AddEventHandler("veh_SR:CheckMoneyForVeh", function(vehicle, price, veh_type)
	...
	MySQL.Async.fetchAll(
		...
		function(pvehicle, affected)
			vRP.prompt({
				player,
				"Er du sikker på, at du vil købe dette køretøj? (godkend/afvis)",
				"",
					...
					if string.lower(answer) == "godkend" then
						if #pvehicle > 0 then
							TriggerClientEvent(
								...
								player,
								{
									text = "Du ejer allerede dette køretøj!",
									type = "warning",
									queue = "global",
									timeout = 3000,
									layout = "centerRight",
									...
									if price > 5000 then
										if vRP.tryFullPayment({ user_id, price }) then
											vRP.getUserIdentity({
												...
												function(identity)
													MySQL.Async.execute(
														...
														{ user_id = user_id, vehicle = vehicle, vehicle_plate = "P " .. identity.registration, veh_type = veh_type }
													)
												end,
											})
											...
											TriggerClientEvent(
												...
												player,
												{
													text = "Betalte " .. format_thousand(price) .. " DKK",
													type = "info",
													queue = "global",
													timeout = 3000,
													layout = "centerRight",
											...
											PerformHttpRequest(
												...
												json.encode({
													...
													content = "**" .. user_id .. "** har lige købt en **" .. vehicle .. "** for: **" .. format_thousand(
														price
													) .. "**",
												}),
						...
						TriggerClientEvent(
							...
							player,
							{
								text = "Betalingen afbrudt, da du ikke godkendte.",
								type = "error",
								queue = "global",
								timeout = 5000,
								layout = "centerRight",
