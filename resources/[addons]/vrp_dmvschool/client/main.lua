...
function startintro()
	if schoolLock then
		TriggerEvent(
			...
			{
				text = "<b style='color:red'>Køreskolen holder lukket lige nu.</b>",
				type = "error",
				timeout = 4000,
				layout = "center",
				queue = "global",
...
function startttest()
	if theorylock then
		TriggerEvent(
			"pNotify:SendNotification",
			{
				text = "<b style='color:red'>Du har ikke færdiggjort introduktionen!</b>",
				type = "error",
				timeout = 4000,
				layout = "center",
				queue = "global",
...
function buylicense()
	if testlock then
		TriggerEvent(
			"pNotify:SendNotification",
			{
				text = "<b style='color:red'>Du har ikke færdiggjort introduktionen og teoriprøven!</b>",
				type = "error",
				timeout = 4000,
				layout = "center",
				queue = "global",
...
AddEventHandler("dmv:buylicense", function()
	...
	TriggerEvent(
		"pNotify:SendNotification",
		{
			text = "Du har hermed fået et kørekort, tillykke!",
			type = "success",
			timeout = 5000,
			layout = "center",
			queue = "global",
...
function EndDTest()
	if Error > maxErrors then
		TriggerEvent(
			"pNotify:SendNotification",
			{
				text = "Du dumpede køreprøven med <font style='color:#f90000'>" .. Error .. " fejl!</font>",
				type = "error",
				timeout = 5000,
				layout = "center",
				queue = "global",
		...
		TriggerEvent(
			"pNotify:SendNotification",
			{
				text = "Du bestod køreprøven med <font style='color:#4E9350'>" .. Error .. " fejl</font>, tillykke!",
				type = "success",
				timeout = 5000,
				layout = "center",
				queue = "global",
...
function DIntro()
	...
	TriggerEvent(
		"pNotify:SendNotification",
		{
			text = "Introduktionen er nu afsluttet.",
			type = "success",
			timeout = 4000,
			layout = "bottomCenter",
			queue = "global",
...
Citizen.CreateThread(function()
	while true do
		...
		elseif
			(IsPedInAnyVehicle(GetPlayerPed(-1), false))
			and SpeedControl == 3
			and CarSpeed >= speed_limit_freeway
			and onPtest == true
		then
			TriggerEvent("pNotify:SendNotification", {
...
Citizen.CreateThread(function()
	while true do
		...
		local float
		Max = GetVehicleMaxSpeed(vehicleModel)
