...
AddEventHandler("es_bank:toofar", function(robb)
	if robbers[source] then
		...
		TriggerClientEvent(
			...
			-1,
			{
				text = "Røveriet blev aflyst: " .. banks[robb].nameofbank,
				type = "warning",
				queue = "global",
				timeout = 5000,
				layout = "centerRight",
...
AddEventHandler("es_bank:rob", function(robb)
	...
	if vRP.hasPermission({ user_id, cfg.permission }) then
		TriggerClientEvent(
			...
			player,
			{
				text = "Politiet kan ikke røve banke.",
				type = "warning",
				queue = "global",
				timeout = 5000,
				layout = "centerRight",
			...
			if banks[robb] then
				...
				if (os.time() - bank.lastrobbed) < cfg.seconds + cfg.cooldown and bank.lastrobbed ~= 0 then
					TriggerClientEvent(
						...
						player,
						{
							text = "Denne bank er for nyligt blevet røvet, venligst vent: "
								.. (cfg.seconds + cfg.cooldown - (os.time() - bank.lastrobbed))
								.. "^0 seconds.",
							type = "warning",
							timeout = 5000,
							layout = "cenerLeft",
				...
				TriggerClientEvent(
					...
					-1,
					{
						text = "Røveri igang ved " .. bank.nameofbank,
						type = "warning",
						queue = "global",
						timeout = 5000,
						layout = "centerRight",
				...
				TriggerClientEvent(
					...
					player,
					{
						text = "Du har startet et bankrøveri ved: " .. bank.nameofbank .. ", kom ikke for langt væk fra boksen!",
						type = "warning",
						timeout = 5000,
						layout = "centerRight",
						animation = { open = "gta_effects_fade_in", close = "gta_effects_fade_out" },
				...
				TriggerClientEvent(
					...
					player,
					{
						text = "Beskyt banken imens pengene bliver tømt 10 minutter og pengene er dine!",
						type = "warning",
						timeout = 5000,
						layout = "centerRight",
						animation = { open = "gta_effects_fade_in", close = "gta_effects_fade_out" },
					...
					if robbers[savedSource] then
						if user_id then
							...
							TriggerClientEvent(
								...
								-1,
								{
									text = "Røveriet er ovre: " .. bank.nameofbank .. "!",
									type = "warning",
									queue = "global",
									timeout = 5000,
									layout = "centerRight",
