...
Citizen.CreateThread(function()
	while true do
		...
		for k, v in pairs(doors) do
			...
			if
				GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.coords[1], v.coords[2], v.coords[3], true) <= lradius
			then
				local closeDoor =
					GetClosestObjectOfType(v.coords[1], v.coords[2], v.coords[3], lradius, v.hash, false, false, false)
				if closeDoor ~= 0 then
					if v.locked then
...
function toggleClosestDoor()
	...
	for k, v in pairs(doors) do
		if v.close ~= nil then
			local door1 =
				GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), v.coords[1], v.coords[2], v.coords[3], true)
			local lradius = (v.range ~= nil and v.range or 2.0)
			if door1 < lradius then
				if
					...
					< GetDistanceBetweenCoords(
						...
						y,
						z,
						doors[v.close].coords[1],
						doors[v.close].coords[2],
						doors[v.close].coords[3],
						true
					)
				then
