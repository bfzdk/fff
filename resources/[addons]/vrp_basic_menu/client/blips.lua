...
AddEventHandler("showBlips", function()
	...
	if showblip then
		showsprite = true
		vRP.notify({ "~g~Blips til" })
	else
		showsprite = false
		vRP.notify({ "~r~Blips fra" })
	end
end)
				...
				if showblip then
					...
					if not DoesBlipExist(blip) then -- Add blip and create head display on player
						...
						elseif veh then
							...
							elseif
								vehModel == GetHashKey("insurgent")
								or vehModel == GetHashKey("insurgent2")
								or vehModel == GetHashKey("limo2")
							then -- insurgent (+ turreted limo cuz limo blip wont work)
