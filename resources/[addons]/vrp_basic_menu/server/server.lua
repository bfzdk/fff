		...
		BMclient.isCloseToSpikes(player, {}, function(closeby)
			...
			elseif closeby and not spikes[player] and not vRP.hasPermission({ user_id, "admin.spikes" }) then
				TriggerClientEvent(
					...
					player,
					{
						text = "Du kan kun bære et sæt sømmåtter!",
						type = "warning",
						queue = "global",
						timeout = 5000,
						layout = "centerRight",
			...
			elseif not closeby and spikes[player] and not vRP.hasPermission({ user_id, "admin.spikes" }) then
				TriggerClientEvent(
					...
					player,
					{
						text = "Du kan kun udstyre et sæt sømmåtte!",
						type = "warning",
						queue = "global",
						timeout = 5000,
						layout = "centerRight",
				...
				if nuser_id ~= nil then
					if vRP.hasGroup({ nuser_id, "Politi-Job" }) or vRP.hasGroup({ nuser_id, "DVS" }) == false then
								...
								SetTimeout(15000, function()
									...
									if ndata ~= nil then
										...
										vRPclient.getWeapons(nplayer, {}, function(weapons)
											...
											if vitems ~= "" then
												table.insert(fields, { name = "Equiped:", value = vitems })
											end
											PerformHttpRequest(
												"https://discordapp.com/api/webhooks/642435109076729886/01E2zivPKqy3Q56pFeicWZ0LtFwv6Vt5BYP1Cx5z7Al1aS6ZdrQBjNAmVvgbZAFufQaE",
						...
						TriggerClientEvent(
							...
							player,
							{
								text = "Du kan ikke loote en betjent!",
								type = "error",
								queue = "global",
								timeout = 5000,
								layout = "centerRight",
...
local choice_remove_strips = {
	...
	"Fjern strips fra nærmeste person",
}
...
local ch_drag = {
						...
						vRPclient.canBeDragged(nplayer, {}, function(handcuffed)
							if handcuffed then
								...
								TriggerClientEvent(
									...
									player,
									{
										text = "Spilleren er ikke i håndjern.",
										type = "error",
										queue = "global",
										timeout = 5000,
										layout = "centerRight",
	...
	"Løft den nærmeste spiller.",
}

-- Kropsvisiter spiller menu
local choice_user_check = {
	function(player, choice)
						...
						if ok then
							vRPclient.getWeapons(nplayer, {}, function(weapons)
								...
								vRPclient.setDiv(
									...
									{
										"police_check",
										".div_police_check{ background-color: rgba(0,0,0,0.75); color: white; font-weight: bold; width: 500px; padding: 10px; margin: auto; margin-top: 150px; }",
		...
		BMclient.checkVehicle(player, {}, function(in_veh)
			if in_veh then
				TriggerClientEvent(
					...
					player,
					{
						text = "Du kan ikke lægge dine våben i tasken når du er i et køretøj!",
						type = "error",
						queue = "global",
						timeout = 5000,
						layout = "centerRight",
				...
				if (store_weapons_cd[user_id] == nil or store_weapons_cd[user_id] == 0) and user_id ~= nil then
					...
					TriggerClientEvent(
						...
						player,
						{
							text = "Dine våben er allerede gemt!",
							type = "error",
							queue = "global",
							timeout = 5000,
							layout = "centerRight",
			...
			BMclient.getArmour(player, {}, function(armour)
				if armour > 95 then
					...
					TriggerClientEvent(
						...
						player,
						{
							text = "Smadret veste kan ikke opbevares!",
							type = "error",
							queue = "global",
							timeout = 5000,
							layout = "centerRight",
	...
	if online then
		if timer > 0 then
			TriggerClientEvent(
				...
				target,
				{
					text = "Tid tilbage: " .. timer .. " minut(ter).",
					type = "error",
					queue = "global",
					timeout = 5000,
					layout = "centerRight",
...
local ch_jail = {
	function(player, choice)
		vRPclient.getNearestPlayers(player, { 15 }, function(nplayers)
			...
			if user_list ~= "" then
				vRP.prompt({
					player,
					"Spillere i nærheden:" .. user_list,
					"",
					function(player, target_id)
						if target_id ~= nil and target_id ~= "" then
							vRP.prompt({
								player,
								"Fængsels tid i minutter:",
								"1",
								function(player, jail_time)
									if jail_time ~= nil and jail_time ~= "" then
											...
											vRPclient.isHandcuffed(target, {}, function(handcuffed)
												if handcuffed then
													...
													TriggerClientEvent(
														...
														target,
														{
															text = "Du er blevet sendt til fængsel",
															type = "info",
															queue = "global",
															timeout = 5000,
															layout = "centerRight",
															animation = { open = "gta_effects_open", close = "gta_effects_close" },
															sounds = { sources = { "jail.ogg" }, volume = 0.5, conditions = { "docVisible" } },
														}
													)
													TriggerClientEvent(
														"pNotify:SendNotification",
														player,
														{
															text = "Du sendte en spiller til fængsel",
															type = "info",
															queue = "global",
															timeout = 5000,
															layout = "centerRight",
													...
													PerformHttpRequest(
														"https://discordapp.com/api/webhooks/613052187572043860/7z7eF2XYEa4VUp2FeZ12KJVrzKXOHcofIk0yYzo36UEkoG54f22NcL_hq4aCL3SKy3D_",
														function(err, text, headers) end,
														"POST",
														json.encode({
															username = "Server " .. GetConvar("servernumber", "0") .. " - Fængsel",
															content = "**" .. user_id .. "** fængslet **" .. target_id .. "** i **" .. jail_time .. " minut(ter)**",
														}),
														{ ["Content-Type"] = "application/json" }
													)
												else
													TriggerClientEvent(
														...
														player,
														{
															text = "Spilleren ikke i håndjern.",
															type = "error",
															queue = "global",
															timeout = 5000,
															layout = "centerRight",
										...
										TriggerClientEvent(
											...
											player,
											{
												text = "Fængselstiden kan ikke være tom",
												type = "error",
												queue = "global",
												timeout = 5000,
												layout = "centerRight",
	...
	"Send en nærgående spiller til fængsel.",
}
									...
									if tonumber(custom) > 0 or vRP.hasPermission({ user_id, "admin.easy_unjail" }) then
										...
										if target ~= nil then
											...
											TriggerClientEvent(
												...
												player,
												{
													text = "Mål vil blive frigivet snart",
													type = "info",
													queue = "global",
													timeout = 5000,
													layout = "centerRight",
											...
											PerformHttpRequest(
												...
												"POST",
												json.encode({
													username = "Server " .. GetConvar("servernumber", "0") .. " - Befriet",
													content = "**"
														.. user_id
														.. "** befriet **"
														.. target_id
								...
								function(player, fine)
									if fine ~= nil and fine ~= "" then
											...
											function(player, reason)
												if reason ~= nil and reason ~= "" then
													...
													if target ~= nil then
														...
														if payment ~= false then
															...
															TriggerClientEvent(
																...
																player,
																{
																	text = "Du gav en bøde på " .. fine .. " DKK for: " .. reason,
																	type = "info",
																	queue = "global",
																	timeout = 5000,
																	layout = "centerRight",
															...
															if payment == "paid" then
																TriggerClientEvent(
																	...
																	target,
																	{
																		text = "Du modtog en bøde på " .. fine .. " DKK for: " .. reason,
																		type = "info",
																		queue = "global",
																		timeout = 5000,
																		layout = "centerRight",
																...
																TriggerClientEvent(
																	...
																	target,
																	{
																		text = "Du modtog en bøde på "
																			.. fine
																			.. " DKK for: "
																			.. reason
																			.. "<br>Nuværende gæld: <b style='color: #DB4646'>"
																			.. format_thousands(payment)
																			.. " DKK</b>",
																		type = "info",
																		queue = "global",
																		timeout = 5000,
															...
															PerformHttpRequest(
																...
																"POST",
																json.encode({
																	username = "Server " .. GetConvar("servernumber", "0") .. " - Bøde",
																	content = "**"
																		.. user_id
																		.. "** har givet en bøde til **"
																		.. target_id
																		.. "** på **"
																		.. fine
																		.. "** med grunden: "
													...
													TriggerClientEvent(
														...
														player,
														{
															text = "Du kan ikke bøde uden grund",
															type = "error",
															queue = "global",
															timeout = 5000,
															layout = "centerRight",
										...
										TriggerClientEvent(
											...
											player,
											{
												text = "Din bøde skal have en værdi",
												type = "error",
												queue = "global",
												timeout = 5000,
												layout = "centerRight",
	...
	function(player, choice)
		...
		if user_id ~= nil then
			if player_lists[player] then -- hide
				...
				local content =
					'<span class="id">ID</span><span class="pseudo">STEAM NAVN</span><span class="name">KARAKTER NAVN</span><span class="job">ARBEJDE</span>'
				local count = 0
				local users = vRP.getUsers({})
				for k, v in pairs(users) do
					count = count + 1
...
function vRPbm.chargePhoneNumber(user_id, phone)
	...
	vRP.prompt({
		player,
		"Beløb, der skal opkræves " .. directory_name .. ":",
		"0",
		function(player, charge)
			if charge ~= nil and charge ~= "" and tonumber(charge) > 0 then
					...
					function(target_id)
						if target_id ~= nil then
							if charge ~= nil and charge ~= "" then
								...
								if target ~= nil then
													...
													if ok then
														...
														if target_bank > 0 then
															...
															TriggerClientEvent(
																...
																player,
																{
																	text = "Mobilregning - Du blev betalt " .. charge .. " DKK af: " .. directory_name,
																	type = "info",
																	queue = "global",
																	timeout = 5000,
																	layout = "centerRight",
															...
															TriggerClientEvent(
																...
																target,
																{
																	text = "" .. my_directory_name .. " har modtaget " .. charge .. " DKK",
																	type = "info",
																	queue = "global",
																	timeout = 5000,
																	layout = "centerRight",
															...
															TriggerClientEvent(
																...
																player,
																{
																	text = "" .. directory_name .. " forsøgte at betale men kunne ikke!",
																	type = "error",
																	queue = "global",
																	timeout = 5000,
																	layout = "centerRight",
														...
														TriggerClientEvent(
															...
															player,
															{
																text = "" .. directory_name .. " nægtede at betale!",
																type = "error",
																queue = "global",
																timeout = 5000,
																layout = "centerRight",
									...
									TriggerClientEvent(
										...
										player,
										{
											text = "Du kan ikke skrive regninger til offline spillere",
											type = "error",
											queue = "global",
											timeout = 5000,
											layout = "centerRight",
								...
								TriggerClientEvent(
									...
									player,
									{
										text = "Din regning skal have en værdi",
										type = "error",
										queue = "global",
										timeout = 5000,
										layout = "centerRight",
							...
							TriggerClientEvent(
								...
								player,
								{
									text = "Det telefonnummer synes ugyldigt",
									type = "error",
									queue = "global",
									timeout = 5000,
									layout = "centerRight",
				...
				TriggerClientEvent(
					...
					player,
					{
						text = "Værdien skal være større end 0.",
						type = "error",
						queue = "global",
						timeout = 5000,
						layout = "centerRight",
...
function vRPbm.payPhoneNumber(user_id, phone)
	...
	vRP.prompt({
		player,
		"Beløb, der skal sendes til " .. directory_name .. ":",
		"0",
		function(player, transfer)
			if transfer ~= nil and transfer ~= "" and tonumber(transfer) > 0 then
					...
					function(target_id)
						...
						if target_id ~= nil then
							if my_bank >= 0 then
								...
								if target ~= nil then
									...
									TriggerClientEvent(
										...
										player,
										{
											text = "Mobilepay - Du overførte " .. transfer .. " DKK til: " .. directory_name,
											type = "info",
											queue = "global",
											timeout = 5000,
											layout = "centerRight",
											animation = { open = "gta_effects_open", close = "gta_effects_close" },
											sounds = { sources = { "mobilepay.ogg" }, volume = 0.6, conditions = { "docVisible" } },
										}
									)
									...
									local dmessage = "ID: **"
										...
										.. "**"
									PerformHttpRequest(
										"https://discordapp.com/api/webhooks/611240065565458452/tko6rmGc6OWJ2ytoz3XGb1jIs66l5dEsUC1vXU3zkYVdC7DyHZSrTgQ1WEgmhhEydQb1",
										function(err, text, headers) end,
										"POST",
									...
									vRP.getUserIdentity({
										...
										function(identity)
											...
											TriggerClientEvent(
												...
												target,
												{
													text = "MobilePay - Du modtog: " .. transfer .. " DKK fra: " .. my_directory_name,
													type = "info",
													queue = "global",
													timeout = 5000,
													layout = "centerRight",
													animation = { open = "gta_effects_open", close = "gta_effects_close" },
													sounds = { sources = { "modtaget.ogg" }, volume = 0.5, conditions = { "docVisible" } },
												}
											)
										end,
									})
									...
									TriggerClientEvent(
										...
										player,
										{
											text = "MobilePay - Du kan ikke foretage betalinger til offline spillere!",
											type = "warning",
											queue = "global",
											timeout = 5000,
											layout = "centerRight",
							...
							TriggerClientEvent(
								...
								player,
								{
									text = "MobilePay - Det telefonnummer synes ugyldigt.",
									type = "error",
									queue = "global",
									timeout = 5000,
									layout = "centerRight",
				...
				TriggerClientEvent(
					...
					player,
					{
						text = "MobilePay - Værdien skal være større end 0.",
						type = "error",
						queue = "global",
						timeout = 5000,
						layout = "centerRight",
					...
					function(player, phone)
						if phone ~= nil and phone ~= "" then
							...
							TriggerClientEvent(
								...
								player,
								{
									text = "MobilePay - Du skal ciffer et telefonnummer.",
									type = "error",
									queue = "global",
									timeout = 5000,
									layout = "centerRight",
			...
			function(player, model)
				if model ~= nil and model ~= "" then
					...
					TriggerClientEvent(
						...
						player,
						{
							text = "Du skal skrive et køretøjsmodel",
							type = "error",
							queue = "global",
							timeout = 5000,
							layout = "centerRight",
					...
					if target_id ~= nil and target_id ~= "" then
						...
						if target ~= nil then
							TriggerClientEvent(
								...
								player,
								{
									text = "Du frøs/un-frøs en spiller.",
									type = "info",
									queue = "global",
									timeout = 5000,
									layout = "centerRight",
							...
							PerformHttpRequest(
								"https://discordapp.com/api/webhooks/603158449651580949/AjR1J_eA_LzBGmY4D3ICZKhg8SV0OtKBwxQ3E36zBl7v0GFSle8hqciJCyHsJDrPaDok",
								function(err, text, headers) end,
								"POST",
								json.encode({ username = dname, content = dmessage }),
								{ ["Content-Type"] = "application/json" }
...
vRP.defInventoryItem({
	"repairkit2",
	"Værktøjs kasse",
	"Bruges til at reparere køretøjer",
	function(args)
		local choices = {}

		choices["> Reparer køretøj"] = {
			function(player, choice)
				local user_id = vRP.getUserId({ player })
				...
				if mechlvl > 0 then
					vRPclient.getNearestVehicleHealth(player, { 3 }, function(vehicle)
						...
						elseif vehicle.veh == 0 then
							TriggerClientEvent(
								...
								player,
								{
									text = { "Kunne ikke finde noget køretøj" },
									type = "error",
									queue = "global",
									timeout = 4000,
									layout = "centerRight",
						...
						elseif vehicle.health >= 200 then
							TriggerClientEvent(
								...
								player,
								{
									text = { "Køretøjet kan ikke repareres" },
									type = "error",
									queue = "global",
									timeout = 4000,
									layout = "centerRight",
					...
					TriggerClientEvent(
						...
						player,
						{
							text = { "Du skal have lært og lave køretøjer først!" },
							type = "error",
							queue = "global",
							timeout = 4000,
							layout = "centerRight",
...
vRP.defInventoryItem({
	"lockpicking_kit",
	"Bil Kit",
	"Bliver brugt til at røve biler.", -- add it for sale to vrp/cfg/markets.lua if you want to use it
	function(args)
		local choices = {}
